...
public class ActorAppearanceWindow : Window, IDisposable
{
    ...
    public unsafe void DrawBody(ActorAppearanceCapability? actorAppearance)
    {
        ...
        ImStarfallStudio.ButtonSelectorStrip("appearance_filters_selector", new Vector2(ImStarfallStudio.GetRemainingWidth(), ImStarfallStudio.GetLineHeight()), ref selected, ["Equipment", "Customize"]);
        ...
        if(selected == 1)
        {
            ...
            catch(Exception ex) { StarfallStudio.Log.Error(ex, "Error drawing customize pane"); }
        }
        else
        {
    ...
    private void DrawHeader()
    {
        ...
        if(ImStarfallStudio.Button("Load NPC", FontAwesomeIcon.PersonArrowDownToLine, buttonSize, centerTest: true))
        {
        ...
        {
            if(ImStarfallStudio.Button("Revert", FontAwesomeIcon.RedoAlt, buttonSize, centerTest: true))
                _ = _capability.ResetAppearance();
        }

        ImGui.SameLine();

        if(ImStarfallStudio.Button("Redraw", FontAwesomeIcon.PaintBrush, buttonSize, centerTest: true))
        ...
        {
            if(ImStarfallStudio.FontIconButton("toggle_adv_bar", isAdvancedMenuOpen ? FontAwesomeIcon.ArrowUp : FontAwesomeIcon.ArrowDown, "Toggle Advanced Menu"))
            {
        ...
        if(ImStarfallStudio.FontIconButtonRight("import", FontAwesomeIcon.FileDownload, 2, "Import Character"))
        ...
        if(ImStarfallStudio.FontIconButtonRight("export", FontAwesomeIcon.Save, 3, "Save Character File"))
    ...
    {
        if(ImStarfallStudio.FontIconButtonRight("import_options", FontAwesomeIcon.Cog, 1, "Import Options"))
