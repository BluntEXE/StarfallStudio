...
public class PosingGraphicalWindow : Window, IDisposable
{
    ...
    public unsafe override void PreDraw()
    {
        ...
        if(_configurationService.Configuration.Appearance.EnableStarfallStudioScale)
        {
    ...
    public override void Draw()
    {
        ...
        if(posing.TransformWindowOpen || _hideControlPane)
            leftPanelWidth = ImStarfallStudio.GetRemainingWidth() - ImGui.GetStyle().ItemSpacing.X;
        else
            leftPanelWidth = ImStarfallStudio.GetRemainingWidth() - RightPanelWidth - ImGui.GetStyle().ItemSpacing.X;
            ...
            ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse | ImGuiWindowFlags.NoBackground))
        {
            if(rightPane.Success && posing.TransformWindowOpen is false && _hideControlPane is false)
            {
                ...
                float height = ImStarfallStudio.GetRemainingHeight() - ImStarfallStudio.GetLineHeight() - (ImGui.GetStyle().FramePadding.Y * 2);
    ...
    private void DrawGlobalButtons(PosingCapability posing)
    {
        ...
        if(ImStarfallStudio.ToggelButton("Freeze Physics", new Vector2(95, 0), _physicsService.IsFreezeEnabled, hoverText: _physicsService.IsFreezeEnabled ? "Un-Freeze Physics" : "Freeze Physics"))
        {
        ...
        {
            if(ImStarfallStudio.ToggelButton("Freeze Character", new Vector2(110, 0), capability.SpeedMultiplier == 0, hoverText: capability.SpeedMultiplierOverride == 0 ? "Un-Freeze Character" : "Freeze Character") || InputManagerService.ActionKeysPressedLastFrame(InputAction.Posing_Freeze))
            {
        ...
        ImStarfallStudio.ButtonSelectorStrip("posing_page_selector", new(ImStarfallStudio.GetRemainingWidth() - RightPanelWidth - 10, ImStarfallStudio.GetLineHeight()), ref _selectedPane, _bonePages);

        ImGui.SameLine();

        ImStarfallStudio.RightAlign(buttonWidth, 8);

        if(ImStarfallStudio.FontIconButton((posing.OverlayOpen ? FontAwesomeIcon.EyeSlash : FontAwesomeIcon.Eye), new(buttonWidth, 0)))
        ...
        {
            if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.LocationCrosshairs, new(buttonWidth, 0)))
        ...
        if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.Search, new(buttonWidth, 0)))
        ...
        ImStarfallStudio.RightAlign(buttonWidth, 4);

        using(ImRaii.Disabled(!posing.CanUndo))
        {
            if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.Backward, new(buttonWidth, 0)) || (InputManagerService.ActionKeysPressedLastFrame(InputAction.Posing_Undo) && posing.CanUndo))
                posing.Undo();
        }
        ...
        {
            if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.Forward, new(buttonWidth, 0)) || (InputManagerService.ActionKeysPressedLastFrame(InputAction.Posing_Redo) && posing.CanRedo))
                posing.Redo();
        }
        ...
        {
            if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.Undo, new(buttonWidth, 0)))
        ...
        {
            if(ImStarfallStudio.FontIconButton(_hideControlPane ? FontAwesomeIcon.SlidersH : FontAwesomeIcon.SlidersH, new(buttonWidth, 0)))
    ...
    private static void DrawButtons(PosingCapability posing)
    {
        ...
        {
            if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.LevelUpAlt, new Vector2(buttonWidth, 0)))
    ...
    private void DrawImportButtons(PosingCapability posing)
    {
        ...
        if(ImStarfallStudio.Button("Import##import_pose", FontAwesomeIcon.FileDownload, buttonSize))
        ...
        if(ImStarfallStudio.Button("Export##export_pose", FontAwesomeIcon.Save, buttonSize))
    ...
    private unsafe void DrawGizmo()
    {
        ...
        if(ImStarfallStudio.FontIconButton((_posingService.CoordinateMode == PosingCoordinateMode.Local ? FontAwesomeIcon.Globe : FontAwesomeIcon.Atom)))
        ...
        if(ImStarfallStudioGizmo.DrawRotation(ref matrix, gizmoSize, _posingService.CoordinateMode == PosingCoordinateMode.World))
        {
        ...
        if(_trackingMatrix.HasValue)
        {
            ...
            selected.Switch(
                ...
                _ =>
                {
                    if(_groupedSnapshotPending == null && ImStarfallStudioGizmo.IsUsing())
                    {
        ...
        if(!ImStarfallStudioGizmo.IsUsing() && _trackingMatrix.HasValue)
        {
