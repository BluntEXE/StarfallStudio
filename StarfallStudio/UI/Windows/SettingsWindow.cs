...
public class SettingsWindow : Window
{
    ...
    public override void Draw()
    {
        using(ImRaii.PushId("brio_settings"))
        {
            if(_isModal)
            {
                ...
                if(ImStarfallStudio.Button("Close", FontAwesomeIcon.Times, new Vector2(100, 0)))
                {
                    IsOpen = false;
                }
            }
            ...
            {
                ImStarfallStudio.ButtonSelectorStrip("settings_filters_selector", new Vector2(ImStarfallStudio.GetRemainingWidth(), ImStarfallStudio.GetLineHeight()), ref selected, ["General", "IPC", "Posing", "Library", "Auto-Save", "Input", "Advanced"]);
    ...
    private void DrawOpenStarfallStudioSetting()
    {
        var selectedStarfallStudioOpenBehavior = _configurationService.Configuration.Interface.OpenStarfallStudioBehavior;
        const string label = "Open StarfallStudio";
        ImGui.SetNextItemWidth(-ImGui.CalcTextSize(label).X - 20);
        using(var combo = ImRaii.Combo(label, selectedStarfallStudioOpenBehavior.ToString()))
        {
            if(combo.Success)
            {
                foreach(var openStarfallStudioBehavior in Enum.GetValues<OpenStarfallStudioBehavior>())
                {
                    if(ImGui.Selectable($"{openStarfallStudioBehavior}", openStarfallStudioBehavior == selectedStarfallStudioOpenBehavior))
                    {
                        _configurationService.Configuration.Interface.OpenStarfallStudioBehavior = openStarfallStudioBehavior;
                        _configurationService.ApplyChange();
                    }
                }
            }
    ...
    private void DrawDisplaySettings()
    {
        ...
        if(ImGui.Checkbox("Censor Actor Names Across StarfallStudio", ref censorActorNames))
        {
        ...
        bool enableStarfallStudioColor = _configurationService.Configuration.Appearance.EnableStarfallStudioColor;
        if(ImGui.Checkbox("Enable StarfallStudio Color", ref enableStarfallStudioColor))
        {
            _configurationService.Configuration.Appearance.EnableStarfallStudioColor = enableStarfallStudioColor;
            _configurationService.ApplyChange();
        }

        bool enableStarfallStudioScale = _configurationService.Configuration.Appearance.EnableStarfallStudioScale;
        if(ImGui.Checkbox("Enable StarfallStudio Scale", ref enableStarfallStudioScale))
        {
            _configurationService.Configuration.Appearance.EnableStarfallStudioScale = enableStarfallStudioScale;
            _configurationService.ApplyChange();
        }
    }
    ...
    private void DrawThirdPartyIPC()
    {
        if(ImGui.CollapsingHeader("Third-Party", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ...
            using(ImRaii.Disabled(penumbraUnavailable))
            {
                ...
                ImGui.SameLine();
                if(ImStarfallStudio.FontIconButton("refresh_penumbra", FontAwesomeIcon.Sync, "Refresh Penumbra Status"))
                {
                    _penumbraService.CheckStatus(true);
                }
            }
            ...
            using(ImRaii.Disabled(!enableGlamourer))
            {
                ...
                ImGui.SameLine();
                if(ImStarfallStudio.FontIconButton("refresh_glamourer", FontAwesomeIcon.Sync, "Refresh Glamourer Status"))
                {
                    _glamourerService.CheckStatus(true);
                }
            }
            ...
            using(ImRaii.Disabled(!enableCustomizePlus))
            {
                ...
                ImGui.SameLine();
                if(ImStarfallStudio.FontIconButton("refresh_Customize", FontAwesomeIcon.Sync, "Refresh Customize+ Status"))
                {
                    _customizePlusService.CheckStatus(true);
                }
            }
    ...
    private void DrawStarfallStudioIPC()
    {
        if(ImGui.CollapsingHeader("StarfallStudio", ImGuiTreeNodeFlags.DefaultOpen))
        {
            bool enableStarfallStudioIpc = _configurationService.Configuration.IPC.EnableStarfallStudioIPC;
            if(ImGui.Checkbox("Enable StarfallStudio IPC", ref enableStarfallStudioIpc))
            {
                _configurationService.Configuration.IPC.EnableStarfallStudioIPC = enableStarfallStudioIpc;
                _configurationService.ApplyChange();
            }
            ImGui.Text($"StarfallStudio IPC Status: {(enableStarfallStudioIpc ? "Active" : "Inactive")}");

            bool enableWebApi = _configurationService.Configuration.IPC.AllowWebAPI;
            if(ImGui.Checkbox("Enable StarfallStudio API", ref enableWebApi))
            {
                _configurationService.Configuration.IPC.AllowWebAPI = enableWebApi;
                _configurationService.ApplyChange();
            }

            ImGui.Text($"StarfallStudio API Status: {(_webService.IsRunning ? "Active" : "Inactive")}");
        }
    }
    ...
    private void DrawGPoseSection()
    {
        if(ImGui.CollapsingHeader("GPose", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ...
            bool enableStarfallStudioTargetChange = _configurationService.Configuration.Posing.StarfallStudioTargetChangesWithGPose;
            if(ImGui.Checkbox("StarfallStudio Target Changes with GPose Target", ref enableStarfallStudioTargetChange))
            {
                _configurationService.Configuration.Posing.StarfallStudioTargetChangesWithGPose = enableStarfallStudioTargetChange;
                _configurationService.ApplyChange();
            }

            bool enableGPoseTargetChange = _configurationService.Configuration.Posing.GPoseTargetChangesWithStarfallStudio;
            if(ImGui.Checkbox("GPose Target Changes with StarfallStudio Target", ref enableGPoseTargetChange))
            {
                _configurationService.Configuration.Posing.GPoseTargetChangesWithStarfallStudio = enableGPoseTargetChange;
                _configurationService.ApplyChange();
            }
    ...
    private void DrawOverlaySection()
    {
        if(ImGui.CollapsingHeader("Overlay", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ...
            if(ImGui.ColorEdit4("Light Normal Color", ref lightCircleNormalColor, ImGuiColorEditFlags.NoInputs))
            {
                ...
                StarfallStudio.Log.Error($"{ImGui.ColorConvertFloat4ToU32(lightCircleNormalColor)}");
            }
    ...
    private void DrawAdvancedTab()
    {
        ...
        {
            ImGui.SetClipboardText(StarfallStudio.GetDebugInfo());
        }
    ...
    private void DrawKeysTab()
    {
        ...
        using(ImRaii.Disabled(!enableKeybinds))
        {
            ...
            {
                DrawKeyBind(InputAction.Interface_ToggleStarfallStudioWindow);
