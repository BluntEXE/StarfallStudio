...
using StarfallStudio.UI.Entitites;
...
public class ProjectWindow : Window, IDisposable
{
    ...
    public override void Draw()
    {
        ...
        using(ImRaii.PushColor(ImGuiCol.Text, UIConstants.GizmoMagenta))
            ImGui.Text("ATTENTION: Projects made in this version will be incompatible with the next version of StarfallStudio!");

        using(ImRaii.Disabled(_mCDFService.IsApplyingMCDF || _mCDFService.IsSavingMCDF))
        {
            ImStarfallStudio.ButtonSelectorStrip("library_filters_selector", new Vector2(ImStarfallStudio.GetRemainingWidth(), ImStarfallStudio.GetLineHeight()), ref selected, ["Load", "Save"]);
            ...
            else
            {
                ...
                using var child = ImRaii.Child("###new_pane", new Vector2(ImStarfallStudio.GetRemainingWidth(), ImStarfallStudio.GetRemainingHeight()), false);
                ...
                if(child.Success)
                {
                    ...
                    {
                        if(ImStarfallStudio.Button("Save Project", FontAwesomeIcon.Save, new(135, 0), centerTest: true, tooltip: "Save Project"))
                        {
    ...
    public void DrawLoad()
    {
        ...
        using(var child = ImRaii.Child("###left_pane", new Vector2(windowSize.X - InfoPaneWidth, -1), false, ImGuiWindowFlags.NoBackground | ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse))
        {
            ...
            using(var entriesChild = ImRaii.Child("###entries_pane", new Vector2(entriesPaneWidth, entriesPaneHeight), true))
            {
                ...
                int x = 0;
                foreach(var item in _projectSystem.StarfallStudioProjects.Projects)
                {
                    bool selected = false;
            ...
            {
                if(ImStarfallStudio.Button("Load", FontAwesomeIcon.FileImport, new(120, 0), centerTest: true, tooltip: "Load Project"))
                {
        ...
        using(var child = ImRaii.Child("###right_pane", new Vector2(ImStarfallStudio.GetRemainingWidth(), -1), false, ImGuiWindowFlags.NoBackground | ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse))
        {
            if(child.Success == false)
                return;
            ...
            using(ImRaii.Disabled(selectedItem is null))
            {
                ...
                using(ImRaii.Disabled(doDelete == false))
                    if(ImStarfallStudio.Button("Delete", FontAwesomeIcon.Trash, new(120, 0), centerTest: true, tooltip: "Delete Project"))
                    {
    ...
    {
        float itemHeight = (ImStarfallStudio.GetLineHeight() * 3) + ImGui.GetStyle().ItemSpacing.Y;

        float itemTop = ImGui.GetCursorPosY();
        bool isSelectedItem = isSelected;
        ImGui.Selectable($"###settings_source_{id}_selectable", ref isSelectedItem, ImGuiSelectableFlags.None, new(ImStarfallStudio.GetRemainingWidth(), itemHeight));
