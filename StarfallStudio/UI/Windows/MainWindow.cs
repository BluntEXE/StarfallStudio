...
public class MainWindow : Window, IDisposable
{
    ...
    public MainWindow(
        ...
        : base($"★ {StarfallStudio.Name} [{configService.Version}]###starfall_main_window", ImGuiWindowFlags.AlwaysAutoResize)
    {
        Namespace = "starfall_main_namespace";
    ...
    public override void Draw()
    {
        ...
        {
            StarfallStudio.Log.Error(ex, $"Failed to draw entity section: [ {_entityManager?.SelectedEntity?.FriendlyName ?? "Unknown"} ] ");
        }
    }
    ...
    private void DrawHeaderButtons()
    {
        ...
        float line1FinalWidth = ImStarfallStudio.GetRemainingWidth() - ((buttonWidths * 2) + (ImGui.GetStyle().ItemSpacing.X * 2) + ImGui.GetStyle().WindowBorderSize);
        ...
        using(ImRaii.Disabled(_gPoseService.IsGPosing == false))
        {
            // This fixes a bug with text scaling
            {
                ...
                ImStarfallStudio.Button("0000", FontAwesomeIcon.Bug, new Vector2(0, 0));
                ImGui.SetCursorPos(startPos);
            }

            if(ImStarfallStudio.Button("Project", FontAwesomeIcon.FolderOpen, new Vector2(line1Width, 0), centerTest: true))
                ImGui.OpenPopup("DrawProjectPopup");

            ImGui.SameLine();
            if(ImStarfallStudio.Button("Library", FontAwesomeIcon.Book, new Vector2(line1Width, 0), centerTest: true))
                _libraryWindow.Toggle();
        }

        ImGui.SameLine();
        if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.InfoCircle, new(buttonWidths, 0)))
            _infoWindow.Toggle();
        ...
        ImGui.SameLine();
        if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.Cog, new(buttonWidths, 0)))
