...
using StarfallStudio.Library.Tags;
...
public class LibraryWindow : Window
{
    ...
    private void DrawLibrary()
    {
        using(ImRaii.PushId("##starfall_library"))
        {
            ...
            if(_isModal)
            {
                ...
                if(ImStarfallStudio.Button("Browse for File", FontAwesomeIcon.FolderOpen, new Vector2(155, 0)))
                    DoBrowse();
                ...
                ImGuiWindowFlags.NoBackground | ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse))
            {
                ...
                float entriesPaneHeight = ImStarfallStudio.GetRemainingHeight() - ImStarfallStudio.GetLineHeight() - ImGui.GetStyle().ItemSpacing.Y;
                float entriesPaneWidth = ImStarfallStudio.GetRemainingWidth();
            ...
            using(var child = ImRaii.Child("###right_pane", new Vector2(ImStarfallStudio.GetRemainingWidth(), -1), false,
                ImGuiWindowFlags.NoBackground | ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse))
            {
                if(child.Success == false)
                    return;

                float paneHeight = ImStarfallStudio.GetRemainingHeight() - (ImStarfallStudio.GetLineHeight() + ImGui.GetStyle().ItemSpacing.Y);

                using(var child2 = ImRaii.Child("###library_info_pane", new Vector2(ImStarfallStudio.GetRemainingWidth(), paneHeight), true,
                ...
                if(_isModal)
                {
                    ...
                    if(_selected is ItemEntryBase ieb)
                    {
                        ...
                        if(ieb is not null)
                        {
                            ...
                            {
                                if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.Heart))
                                {
                                    if(isFavorite)
                    ...
                    ImGui.SetCursorPosX(ImGui.GetCursorPosX() + (ImStarfallStudio.GetRemainingWidth() - (offset + ImGui.GetStyle().ItemSpacing.X)));

                    if(isPoseModal)
                    {
                        if(ImStarfallStudio.Button("##importPoseOptionButton", FontAwesomeIcon.Cog, new Vector2(25, 0), tooltip: "Import Options"))
                        {
                    ...
                    if(ImStarfallStudio.Button("Import", FontAwesomeIcon.Check, new Vector2(100, 0)))
                    {
                        if(_selected != null)
                        {
                            OnOpen(_selected);
                    ...
                    if(ImStarfallStudio.Button("Cancel", FontAwesomeIcon.Times, new Vector2(100, 0)))
                    {
                        Close();
                    }
                }
    ...
    private void DrawFilters()
    {
        ...
        if(_isModal && _modalFilter != null)
        {
            ...
            if(ImStarfallStudio.ButtonSelectorStrip("library_filters_selector", new(ImStarfallStudio.GetRemainingWidth(), ImStarfallStudio.GetLineHeight()), ref selected, [.. ops]))
            {
                if(selected == 0)
                {
                    _selectedFilter = _favoritesFilter;
    ...
    {
        float lineHeight = ImStarfallStudio.GetLineHeight();
        ...
        if(width == -1)
            width = ImStarfallStudio.GetRemainingWidth();
        ...
        try
        {
            using(var child = ImRaii.Child("library_path_input", new(width, lineHeight), false, ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse))
            {
                ...
                using(ImRaii.PushColor(ImGuiCol.Button, 0))
                {
                    ...
                    {
                        if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.CaretUp, new(PathBarButtonWidth, lineHeight)))
                        {
                    ...
                    for(int i = 0; i < _path.Count; i++)
                    {
                        if(i > 0)
                        {
                            ...
                            ImStarfallStudio.FontIcon(FontAwesomeIcon.CaretRight);
                            ImGui.SameLine();
                        }
                    ...
                    // Blank area
                    float blankWidth = ImStarfallStudio.GetRemainingWidth() - PathBarButtonWidth - ImGui.GetStyle().ItemSpacing.X;
                    ...
                    {
                        StarfallStudio.Log.Warning($"<{blankWidth},{lineHeight}>");
                    }

                    // Refresh Button
                    using(ImRaii.Disabled(_isRescanning))
                    {
                        if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.Repeat, new(PathBarButtonWidth, lineHeight)))
                        {
                            ReScan();
                        }
    ...
    {
        float searchBarWidth = ImStarfallStudio.GetRemainingWidth();
        float searchBarHeight = ImStarfallStudio.GetLineHeight();
                    ...
                    using(var child = ImRaii.Child("library_search_input_child", new(searchBarWidth, searchBarHeight), false, ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse))
                    {
                        ...
                        if(child.Success)
                        {
                            ...
                            if(IsSearching)
                            {
                                ...
                                if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.TimesCircle))
                                {
                                    ClearSearchText();
                                    _searchFilter.Clear();
                                    TagFilter.Clear();
                            ...
                            else
                            {
                                ImGui.BeginDisabled();
                                ImGui.PushStyleColor(ImGuiCol.Button, 0x000000);
                                ImStarfallStudio.FontIconButton(FontAwesomeIcon.Search);
                                ImGui.PopStyleColor();
                                ImGui.EndDisabled();
                            }
                            ...
                            if(TagFilter.Tags != null)
                            {
                                ...
                                foreach(Tag tag in TagFilter.Tags)
                                {
                                    ...
                                    if(ImStarfallStudio.DrawTag(tag) == ImStarfallStudio.MouseAction.Left)
                                    {
                                        toRemove = tag;
                                    }
                                }
                            ...
                            ImGui.SameLine();
                            ImGui.SetCursorPosY(0);
                            ImGui.SetNextItemWidth(ImStarfallStudio.GetRemainingWidth());
        ...
        using(ImRaii.PushStyle(ImGuiStyleVar.WindowPadding, new Vector2(7, 7)))
        {
            ...
            ImGuiWindowFlags.NoFocusOnAppearing | ImGuiWindowFlags.ChildWindow);
            {
                if(window.Success)
                {
                    ...
                    {
                        var (selected, action) = ImStarfallStudio.DrawTags(availableTags);
                        if(selected != null && action == ImStarfallStudio.MouseAction.Left)
                        {
                        ...
                        if(trimmedTags > 0)
                        {
                            ImStarfallStudio.Text($"plus {trimmedTags} more tags...", 0x88FFFFFF);
                        }
                    ...
                    {
                        ImStarfallStudio.Text($"Press TAB to filter by tag \"{availableTags[0].DisplayName}\"", 0x88FFFFFF);
                        hasContent = true;
                    ...
                    if(!hasContent)
                    {
                        ImStarfallStudio.Text($"Start typing to search...", 0x88FFFFFF);
                    }
                }
    ...
    private void DrawEntry(EntryBase entry, float width, int id)
    {
        ...
        if(ImGui.Selectable($"###library_entry_{id}_selectable", ref selected, ImGuiSelectableFlags.AllowDoubleClick, size))
        {
            ...
            {
                _toOpen = entry;
                StarfallStudio.Log.Verbose("IsMouseDoubleClicked");
            }
        }
        ...
        if(ImGui.IsItemVisible())
        {
                ...
                ImGuiWindowFlags.NoBackground | ImGuiWindowFlags.NoInputs | ImGuiWindowFlags.NoScrollbar))
            {
                ...
                if(entry.Icon != null)
                {
                    ImStarfallStudio.ImageFit(entry.Icon, iconSize);
                }

                ImStarfallStudio.TextCentered(entry.Name, iconSize.Y);
            }
        }
    }
    ...
    private void DrawInfo(EntryBase entry)
    {
        ...
        {
            using(var child = ImRaii.Child("###library_info_pane", new Vector2(ImStarfallStudio.GetRemainingWidth(), ImStarfallStudio.GetRemainingHeight()), false))
            {
                if(child.Success)
                {
                    entry.DrawInfo(this);
    ...
    private void OnOpen(EntryBase entry)
    {
        ...
        else if(entry is not null and ItemEntryBase itemEntry)
        {
            ...
            {
                StarfallStudio.Log.Error(ex, "Exception while invoking library OnOpen Callback!");
            }
        }
    }
    ...
    public void TryRefresh(bool filter)
    {
        ...
        {
            StarfallStudio.Log.Error(ex, "Exception while Refreshing!");
        }
    }
