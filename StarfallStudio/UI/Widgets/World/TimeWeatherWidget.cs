...
public class TimeWeatherWidget(TimeWeatherCapability weatherCapability) : Widget<TimeWeatherCapability>(weatherCapability)
{
    ...
    public override void DrawBody()
    {
        ...
        if(ImStarfallStudio.FontIconButtonRight("timeLock", isTimeFrozen ? FontAwesomeIcon.Unlock : FontAwesomeIcon.Lock, 1, isTimeFrozen ? "Unlock Time" : "Lock Time", bordered: false))
        ...
        ImStarfallStudio.AttachToolTip("Time of Day");

        var time = false;
        var dragday = false;
        using(ImRaii.ItemWidth((ImStarfallStudio.GetRemainingWidth() / 2) - ImGui.GetStyle().ItemInnerSpacing.X))
        {
            time = ImGui.SliderInt("##time_set"u8, ref minuteOfDay, 0, DayTime - 1, "%.0f"u8);
            ImStarfallStudio.AttachToolTip("Time of Day (In Minutes)");

            ImGui.SameLine();

            dragday = ImGui.SliderInt("##day_set"u8, ref dayOfMonth, 1, 31);
            ImStarfallStudio.AttachToolTip("Day of Month");
        }
        ...
        ImGui.SetCursorPos(unlockPos);
        if(ImStarfallStudio.FontIconButtonRight("weatherLock", isWeatherOverrideEnabledLocked ? FontAwesomeIcon.Unlock : FontAwesomeIcon.Lock, 1, isWeatherOverrideEnabledLocked ? "Unlock Weather" : "Lock Weather", bordered: false))
        ...
        if(ImStarfallStudio.BorderedWeatherGameIcon("current_weather", (WeatherUnion)Capability.EnvironmentService.CurrentWeather, showText: false))
        {
        ...
        ImStarfallStudio.AttachToolTip("Weather ID");
