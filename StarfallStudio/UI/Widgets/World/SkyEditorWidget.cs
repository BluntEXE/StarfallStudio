...
public class SkyEditorWidget(SkyEditorCapability skyEditorCapability) : Widget<SkyEditorCapability>(skyEditorCapability)
{
    ...
    public unsafe override void DrawBody()
    {
        ...
        ImStarfallStudio.ButtonSelectorStrip("stars_filters_selector", new Vector2(ImStarfallStudio.GetRemainingWidth(), ImStarfallStudio.GetLineHeight()), ref selected, ["Sky", "Stars", "Clouds"]);
        ...
        switch(selected)
        {
            case 1:
                ...
                if(ImStarfallStudio.FontIconButtonRight("###resetStars", FontAwesomeIcon.Redo, 1, "Reset Stars", bordered: false, enabled: isStars))
            ...
            case 0:
                ...
                preservedPOS = ImGui.GetCursorPos();
                ImGui.SetCursorPos(unlockPos - new Vector2(0, 4));
                if(ImStarfallStudio.FontIconButtonRight("###resetSky", FontAwesomeIcon.Redo, 1, "Reset Sky", bordered: false, enabled: isSky))
                    Capability.Environment.EnvironmentOverrideState &= ~EnvironmentOverrideState.Sky;
                ImGui.SetCursorPos(preservedPOS);

                if(ImStarfallStudio.BorderedGameTex("##skyTexturePreview", _skyTextureSelector.GetTexturePath(env->EnvState.SkyTextureID)))
                {
                ...
                preservedPOS = ImGui.GetCursorPos();
                ImGui.SetCursorPos(unlockPos - new Vector2(0, 4));
                if(ImStarfallStudio.FontIconButtonRight("###resetLight", FontAwesomeIcon.Redo, 1, "Reset Lighting", bordered: false, enabled: isLighting))
                ...
                ImStarfallStudio.AttachToolTip("Ambient Color");
            ...
            case 2:
                ...
                preservedPOS = ImGui.GetCursorPos();
                ImGui.SetCursorPos(unlockPos - new Vector2(0, 4));
                if(ImStarfallStudio.FontIconButtonRight("###resetClouds", FontAwesomeIcon.Redo, 1, "Reset Clouds", bordered: false, enabled: isClouds))
                    Capability.Environment.EnvironmentOverrideState &= ~EnvironmentOverrideState.Clouds;
                ImGui.SetCursorPos(preservedPOS);

                if(ImStarfallStudio.BorderedGameTex("##cloudTexturePreview", _cloudTextureSelector.GetTexturePath(env->EnvState.Clouds.CloudTexture)))
                {
                    _cloudTextureSelector.Select(new TextureId(env->EnvState.Clouds.CloudTexture));
                    ImGui.OpenPopup("cloud_texture_selector"u8);
                }
                ImStarfallStudio.AttachToolTip("Click to change Cloud Texture");
                ...
                ImStarfallStudio.AttachToolTip("Cloud Texture ID");

                if(ImStarfallStudio.BorderedGameTex("##cloudSideTexturePreview", _cloudSideTextureSelector.GetTexturePath(env->EnvState.Clouds.CloudSideTexture)))
                {
                ...
                ImStarfallStudio.AttachToolTip("Click to change Cloud Side Texture");
