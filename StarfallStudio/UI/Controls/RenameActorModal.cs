...
namespace StarfallStudio.UI.Controls;
                ...
                if(currentActorEntity is not null && currentActorEntity.IsAttached == true)
                {
                    ...
                    using(ImRaii.Disabled(string.IsNullOrEmpty(currentActorName)))
                    {
                        if(ImGui.Button("Save", new(buttonW, 0)))
                        {
                            ...
                            {
                                StarfallStudio.Log.Info($"Renamed {currentActorEntity.FriendlyName} -> {currentActorName}");
                    ...
                    if(ImGui.Button("Reset Name", new(buttonW, 0)))
                    {
                        if(currentActorEntity.IsAttached)
                        {
                            StarfallStudio.Log.Info($"NameReset {currentActorEntity.FriendlyName} -> {currentActorEntity.FriendlyName = string.Empty}");
                        }
                        Close();
                    }
                    ...
                    if(ImGui.IsKeyDown(ImGuiKey.Enter))
                    {
                        if(currentActorEntity.IsAttached)
                        {
                            StarfallStudio.Log.Info($"Renamed {currentActorEntity.FriendlyName} -> {currentActorName} (via Enter Key)");
                            currentActorEntity.FriendlyName = currentActorName;
                        }
                        Close();
                    }
