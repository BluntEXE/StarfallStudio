...
public static partial class ImStarfallStudio
{
    ...
    public static bool DragFloat2V3(string label, ref Vector3 value, float min, float max, string format, bool degrees = false, ImGuiSliderFlags flags = ImGuiSliderFlags.None, float step = 1.0f)
    {
        ...
        {
            // Convert to degrees
            vector2 = new Vector2(StarfallStudioUtilities.RadiansToDegrees(-value.X), StarfallStudioUtilities.RadiansToDegrees(-value.Y));
            changed = ImGui.DragFloat2(label, ref vector2, step, min, max, format, flags);
            if(changed)
            {
                value.X = StarfallStudioUtilities.DegreesToRadians(-vector2.X);
                value.Y = StarfallStudioUtilities.DegreesToRadians(-vector2.Y);
            }
        }
        else
        {
