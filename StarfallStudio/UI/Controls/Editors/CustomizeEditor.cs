...
public class CustomizeEditor()
{
        ...
        using(var customizeGroup = ImRaii.Child("customizeGroup", slotSizes))
        {
            if(customizeGroup.Success)
            {
                ...
                if(_capability.IsHuman)
                {
                    ...
                    var menus = StarfallStudioCharaMakeType.BuildMenus(currentAppearance);
    ...
    private bool DrawReset(ref ActorAppearance currentAppearance, ActorAppearance originalAppearance)
    {
        ...
        if(ImStarfallStudio.FontIconButtonRight("reset_customize", FontAwesomeIcon.Undo, 1, "Reset Customize", customizeChanged))
        {
    ...
    private bool DrawMenus(ref ActorAppearance appearance, StarfallStudioCharaMakeType.MenuCollection menus)
    {
        bool didChange = false;

        bool hasLipColor = menus.GetMenuTypeForCustomize(CustomizeIndex.LipColor) == StarfallStudioCharaMakeType.MenuType.Color;
        ...
        for(int i = 0; i < menus.Menus.Length; ++i)
        {
            ...
            switch(menu.CustomizeIndex)
            {
                ...
                case CustomizeIndex.FaceFeatures:
                    var colorMenu = menus.Menus.Length >= i && menus.Menus[i + 1].Type == StarfallStudioCharaMakeType.MenuType.Color ? menus.Menus[i + 1] : null;
                    if(!featuresDone)
                    {
                        if(colorMenu != null)
                        {
    ...
    private bool DrawHairSelect(ref ActorCustomize customize, StarfallStudioCharaMakeType.Menu menu, string title)
    {
        bool madeChange = false;

        var hairStyles = StarfallStudioHairMakeType.GetHairStyles(customize);
        ...
        if(ImStarfallStudio.BorderedGameIcon(currentHairIdx.ToString(), currentIcon, "Images.UnknownIcon.png", size: IconSize))
        ...
        if(ImStarfallStudio.DrawIconSelectorPopup("hair_style_popup", hairStyles.Where(x => x.FeatureID > 0).Select(x => new ImStarfallStudio.IconSelectorEntry(x.FeatureID, x.Icon)).ToArray(), ref currentHairIdx, columns: 6, iconSize: IconSize))
        {
            customize.HairStyle = (byte)currentHairIdx;
            madeChange |= true;
        }
    ...
    private bool DrawFacePaintSelect(ref ActorCustomize customize, StarfallStudioCharaMakeType.Menu menu, string title)
    {
        bool madeChange = false;

        var facePaints = StarfallStudioHairMakeType.GetFacePaints(customize);
        ...
        if(ImStarfallStudio.BorderedGameIcon(currentFacepaintIdx.ToString(), currentIcon, "Images.Head.png", size: IconSize))
        ...
        if(ImStarfallStudio.DrawIconSelectorPopup("face_paint_popup", facePaints.Where(x => x.FeatureID > 0).Select(x => new ImStarfallStudio.IconSelectorEntry(x.FeatureID, x.Icon)).ToArray(), ref currentFacepaintIdx, columns: 6, iconSize: IconSize, fallbackImage: "Images.Head.png"))
        {
            customize.RealFacepaint = (byte)currentFacepaintIdx;
            madeChange |= true;
        }
    ...
    private bool DrawFeatureSelect(ref ActorCustomize customize, StarfallStudioCharaMakeType.Menu menu, StarfallStudioCharaMakeType.Menu colorMenu)
    {
        bool madeChange = false;

        int currentFeatures = (int)customize.FaceFeatures;

        List<ImStarfallStudio.IconSelectorEntry> entries = [];

        var face = customize.FaceType - 1;
        bool validFace = face >= 0 && face < StarfallStudioCharaMakeType.FaceCount;

        for(int i = 0; i < StarfallStudioCharaMakeType.FaceFeatureCount; ++i)
        {
            uint featureIcon = validFace ? (uint)menu.FacialFeatures[face, i] : 0;
            entries.Add(new ImStarfallStudio.IconSelectorEntry(1 << i, featureIcon));
        }

        // Legacy tattoo
        entries.Add(new ImStarfallStudio.IconSelectorEntry(128, 0, "Images.LegacyTattoo.png"));

        using(ImRaii.PushId($"face_feature_{menu.MenuId}"))
        {
            if(ImStarfallStudio.BorderedGameIcon($"{menu.Title}", entries[0].Icon, "Images.UnknownIcon.png", size: IconSize))
            ...
            if(ImStarfallStudio.DrawIconSelectorPopup("face_feature_popup", [.. entries], ref currentFeatures, columns: 4, iconSize: IconSize, fallbackImage: "Images.Head.png", bitField: true))
            {
                customize.FaceFeatures = (FacialFeature)currentFeatures;
                madeChange |= true;
            }
    ...
    private unsafe bool DrawItemSelector(ref ActorCustomize customize, CustomizeIndex customizeIndex, StarfallStudioCharaMakeType.Menu menu, bool multiItem)
    {
        bool didChange = false;

        int value = customize.Data[(int)customizeIndex];
        var entries = menu.SubParams.Select((x, i) => new ImStarfallStudio.IconSelectorEntry(i + 1, (uint)x)).ToArray();

        uint graphic = entries.FirstOrDefault(x => !multiItem && x.Id == value || multiItem && (x.Id & value) != 0).Icon;

        if(ImStarfallStudio.BorderedGameIcon($"{customizeIndex}_icon", graphic, "Images.UnknownIcon.png", size: IconSize))
        ...
        if(ImStarfallStudio.DrawIconSelectorPopup($"{customizeIndex}_popup", entries, ref value, columns: 6, iconSize: IconSize, fallbackImage: "Images.Head.png", bitField: multiItem))
        {
            customize.Data[(int)customizeIndex] = (byte)value;
            didChange |= true;
        }
    ...
    private bool DrawGenericMenu(ref ActorCustomize customize, StarfallStudioCharaMakeType.Menu menu)
    {
        ...
        {
            StarfallStudioCharaMakeType.MenuType.Numerical => DrawNumericalSelector(ref customize, customizeIndex, title),
            StarfallStudioCharaMakeType.MenuType.List => DrawListSelector(ref customize, customizeIndex, title),
            StarfallStudioCharaMakeType.MenuType.Color => DrawColorSelector(ref customize, customizeIndex, title, true),
            StarfallStudioCharaMakeType.MenuType.ItemSelect => DrawItemSelector(ref customize, customizeIndex, menu, false),
            StarfallStudioCharaMakeType.MenuType.MultiItemSelect => DrawItemSelector(ref customize, customizeIndex, menu, true),
            _ => false,
        };
    }
    ...
    private unsafe bool DrawColorSelector(ref ActorCustomize customize, CustomizeIndex customizeIndex, string title, bool ownOption = false)
    {
        ...
        if(ImStarfallStudio.DrawLabeledColor($"{customizeIndex}_color", value, valueIdx.ToString(), title))
            ImGui.OpenPopup($"{customizeIndex}_popup");
        ...
        if(ImStarfallStudio.DrawPopupColorSelector($"{customizeIndex}_popup", colors, ref valueIdx))
        {
