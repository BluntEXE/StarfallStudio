...
public static class PosingEditorCommon
{
    ...
    {
        if(ImStarfallStudio.FontIconButton("select_all", FontAwesomeIcon.Check, "Select All"))
        {
            filter.EnableAll();
        }
        ...
        if(ImStarfallStudio.FontIconButton("select_none", FontAwesomeIcon.Minus, "Select None"))
        {
            filter.DisableAll();
        }
        ...
        if(posingService is not null)
        {
            ...
            if(ImStarfallStudio.ToggelFontIconButton("keep_gizmo", FontAwesomeIcon.LocationCrosshairs, new(0), posingService.GizmoStaysWhenAllBonesAreDisabled, hoverText: "Keep gizmo active even when all items in the filter are disabled"))
            {
                posingService.GizmoStaysWhenAllBonesAreDisabled = !posingService.GizmoStaysWhenAllBonesAreDisabled;
            }
        }

        var tristateCheckbox = new ImStarfallStudio.TristateCheckbox();
