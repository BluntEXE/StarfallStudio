...
public class LightEditor
{
    ...
    public static unsafe void DrawLightProperties(LightRenderingCapability Capability)
    {
        ...
        if(ImStarfallStudio.ToggelFontIconButton("togglelight", FontAwesomeIcon.Lightbulb, Vector2.Zero, Capability.GameLight.IsVisible, hoverText: Capability.GameLight.IsVisible ? "Turn Light Off" : "Turn Light On"))
        {
        ...
        if(ImStarfallStudio.FontIconButtonRight("reset", FontAwesomeIcon.Undo, 1, "Reset Light Properties", Capability.HasOverride))
        {
            Capability.Reset();
        }
        ...
        if(ImStarfallStudio.ButtonSelectorStrip("light_type", Vector2.Zero, ref Capability.SelectedLightType, ["Spot", "Point", "Flat", "World"]))
        {
        ...
        switch(light->EmissionType)
        {
            case LightType.SpotLight:
                ...
                ImStarfallStudio.AttachToolTip("Spot Light Angle");

                ImStarfallStudio.CenterNextElementWithPadding(15);
                ImGui.SliderFloat("###lightSmothing"u8, ref light->FalloffAngle, 0.0f, 180.0f, "%0.0f Degrees"u8);
                ImStarfallStudio.AttachToolTip("Spot Light Smothing");
                break;
        ...
        ImStarfallStudio.AttachToolTip("Light Color");

        ImStarfallStudio.CenterNextElementWithPadding(15);
        ImGui.DragFloat("###intensity"u8, ref light->Intensity, 0.01f, 0.0f, 100.0f);
        ImStarfallStudio.AttachToolTip("Intensity");
        ...
        if(ImStarfallStudio.ToggleSelecterStrip("shadows_enable", Vector2.Zero, ref bools, ["Character", "Object", "Dynamic"], "Shadows"))
        {
    ...
    public static unsafe void DrawLightTransform(LightTransformCapability Capability, ref bool activeState)
    {
        ...
        if(ImStarfallStudio.FontIconButton($"overlay_{Capability.Entity.Id}", overlayOpen ? FontAwesomeIcon.EyeSlash : FontAwesomeIcon.Eye, overlayOpen ? "Close Overlay" : "Open Overlay"))
        {
        ...
        if(ImStarfallStudio.ToggelFontIconButton($"###togglegizmo_{Capability.Entity.Id}", FontAwesomeIcon.Crosshairs, Vector2.Zero, Capability.IsGismoVisible, hoverText: Capability.IsGismoVisible ? "Diable, Force Gizmo Viable" : "Force Gizmo Viable"))
        {
        ...
        if(ImStarfallStudio.FontIconButton($"undo_{Capability.Entity.Id}", FontAwesomeIcon.Backward, "Undo", Capability.CanUndo) || (InputManagerService.ActionKeysPressedLastFrame(InputAction.Posing_Undo) && Capability.CanUndo))
        {
            Capability.Undo();
        }
        ...
        if(ImStarfallStudio.FontIconButton($"redo_{Capability.Entity.Id}", FontAwesomeIcon.Forward, "Redo", Capability.CanRedo) || (InputManagerService.ActionKeysPressedLastFrame(InputAction.Posing_Redo) && Capability.CanRedo))
        {
            Capability.Redo();
        }
        ...
        if(ImStarfallStudio.FontIconButtonRight($"reset_{Capability.Entity.Id}", FontAwesomeIcon.Undo, 1, "Reset Light Transform", Capability.HasOverride))
        {
            Capability.Reset();
        }
        ...
        if(child1.Success)
        {
            ...
            (var pdidChange, var panyActive) = ImStarfallStudio.DragFloat3($"###_transform_Light_Position_0", ref Capability.position, 0.1f, FontAwesomeIcon.ArrowsUpDownLeftRight, "Position", enableExpanded: false);
            ImStarfallStudio.VerticalPadding(2);
            (var rdidChange, var ranyActive) = ImStarfallStudio.DragFloat3($"###_transform_Light_Rotation_0", ref Capability.rotation, 1f * 10, FontAwesomeIcon.ArrowsSpin, "Rotation", enableExpanded: false);
            ImStarfallStudio.VerticalPadding(2);
            (var sdidChange, var sanyActive) = ImStarfallStudio.DragFloat3($"###_transform_Light_Scale_0", ref Capability.scale, 1f, FontAwesomeIcon.ExpandAlt, "Scale", enableExpanded: false);
