...
using static StarfallStudio.Game.Actor.ActionTimelineService;
...
public class ActionTimelineEditor(CutsceneManager cutsceneManager, GPoseService gPoseService, EntityManager entityManager, PhysicsService physicsService, ConfigurationService configService)
{
    ...
    {
        if(ImStarfallStudio.ToggelButton("Freeze Physics", new Vector2(95, 25), _physicsService.IsFreezeEnabled, hoverText: _physicsService.IsFreezeEnabled ? "Un-Freeze Physics" : "Freeze Physics"))
        {
            _physicsService.FreezeToggle();
        }

        ImGui.SameLine();
        ImStarfallStudio.HorizontalPadding(2);

        ImStarfallStudio.RightAlign(100 * ImGuiHelpers.GlobalScale, 1);
        ...
        if(ImStarfallStudio.FontIconButtonRight("reset", FontAwesomeIcon.Undo, 1, "Reset Animation", _capability.HasOverride))
        {
            _capability.Reset();
        ...
        if(popup.Success)
        {
            ImStarfallStudio.VerticalPadding(1);

            if(ImStarfallStudio.Button("Freeze All Actors", FontAwesomeIcon.Snowflake, new Vector2(180, 0)))
            {
            ...
            if(ImStarfallStudio.Button("  Un-Freeze All Actors", FontAwesomeIcon.Fire, new Vector2(180, 0)))
            {
                foreach(var actor in _entityManager.TryGetAllActors())
                {
                    if(actor.TryGetCapability<ActionTimelineCapability>(out ActionTimelineCapability? atCap))
            ...
            if(ImStarfallStudio.Button("  Play all Animations", FontAwesomeIcon.PlayCircle, new Vector2(180, 0)))
            {
                foreach(var actor in _entityManager.TryGetAllActors())
                {
                    if(actor.TryGetCapability<ActionTimelineCapability>(out ActionTimelineCapability? atCap))
            ...
            if(ImStarfallStudio.Button("  Stop all Animations", FontAwesomeIcon.StopCircle, new Vector2(180, 0)))
            {
                foreach(var actor in _entityManager.TryGetAllActors())
                {
                    if(actor.TryGetCapability<ActionTimelineCapability>(out ActionTimelineCapability? atCap))
    ...
    private void DrawBaseOverride()
    {
        ...
        if(ImStarfallStudio.IsItemConfirmed())
        {
            ApplyBaseOverride(_capability, true);
        }
        ...
        ImGui.SameLine();
        ImStarfallStudio.HorizontalPadding(4);

        if(ImStarfallStudio.FontIconButtonRight("base_play", FontAwesomeIcon.PlayCircle, 3, "Play", _capability.SlotedBaseAnimation != 0))
            ApplyBaseOverride(_capability);

        ImGui.SameLine();

        if(ImStarfallStudio.FontIconButtonRight("base_reset", FontAwesomeIcon.StopCircle, 2, "Stop", _capability.HasBaseOverride))
        {
        ...
        if(ImStarfallStudio.FontIconButtonRight("base_search", FontAwesomeIcon.Search, 1, "Search"))
        {
            _isBaseMode = true;
    ...
    private void DrawBlend()
    {
        ...
        if(ImStarfallStudio.FontIconButtonRight("blend_play", FontAwesomeIcon.PlayCircle, 2, "Play", _capability.SlotedBlendAnimation != 0))
            ApplyBlend(_capability);

        ImGui.SameLine();

        if(ImStarfallStudio.FontIconButtonRight("blend_search", FontAwesomeIcon.Search, 1, "Search"))
        {
            _isBaseMode = false;
            _globalTimelineSelector.Select(null, false);
    ...
    private void DrawSlot(ActionTimelineSlots slot)
    {
        ...
        using(ImRaii.PushId($"slot_{slot}"))
        {
            ...
            if(ImStarfallStudio.FontIconButtonRight("reset", FontAwesomeIcon.Undo, 1, "Reset Speed", _capability.HasSlotSpeedOverride(slot)))
                _capability.ResetSlotSpeedOverride(slot);

            ImGui.SameLine();

            var speed = _capability.GetSlotSpeed(slot);
            if(ImStarfallStudio.FontIconButtonRight("speed_pause", FontAwesomeIcon.PauseCircle, 2, "Pause", speed > 0f))
    ...
    private void DrawOverallSpeed(bool drawAdvanced)
    {
        ...
        if(ImStarfallStudio.FontIconButtonRight("speed_reset", FontAwesomeIcon.Undo, 1, "Reset Speed", _capability.HasSpeedMultiplierOverride))
            _capability.ResetOverallSpeedOverride();

        ImGui.SameLine();

        if(ImStarfallStudio.FontIconButtonRight("speed_pause", FontAwesomeIcon.PauseCircle, 2, "Pause", _capability.SpeedMultiplier != 0f))
        {
            _capability.SetOverallSpeedOverride(0f);
        }
    }
    ...
    private void DrawCutscene()
    {
        ...
        using(ImRaii.Disabled(string.IsNullOrEmpty(_cameraPath)))
        {
            ...
            {
                if(ImStarfallStudio.Button("Play", FontAwesomeIcon.Play, new Vector2(-1, 30), centerTest: true))
                {
            ...
            using(ImRaii.Disabled(!isrunning))
            {
                if(ImStarfallStudio.Button("Stop", FontAwesomeIcon.Stop, new Vector2(-1, 30), centerTest: true))
                {
                    _cutsceneManager.StopPlayback();
                }
            }
