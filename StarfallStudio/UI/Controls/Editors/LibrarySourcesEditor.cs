...
using static StarfallStudio.Config.LibraryConfiguration;
...
public static class LibrarySourcesEditor
{
    ...
    public static void Draw(string? label, ConfigurationService service, LibraryConfiguration config, float? heightPadding = null)
    {
        ...
        using(ImRaii.Disabled(selectedItem is null || selectedItem.CanEdit == false))
        {
            // Remove Item
            {
                if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.Minus, new(buttonWidth, 0)))
                {
                    // TODO: confirm?
            ...
            // Toggle Item
            {
                ImGui.SetCursorPosX(ImGui.GetCursorPosX() + (ImStarfallStudio.GetRemainingWidth() - (buttonWidth * 3) - (ImGui.GetStyle().ItemSpacing.X * 2)));
                if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.ToggleOff, new(buttonWidth, 0)))
                {
            ...
            // Edit Item
            {
                if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.Edit, new(buttonWidth, 0)))
                {
                    isEditing = true;
        ...
        ImGui.SameLine();
        ImGui.SetCursorPosX(ImGui.GetCursorPosX() + (ImStarfallStudio.GetRemainingWidth() - buttonWidth));

        // New Item
        {
            if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.Plus, new(buttonWidth, 0)))
            {
                isEditing = true;
                isItemEditorOpen = true;
                isNewItem = true;
        ...
        if(isEditing && selectedItem is not null)
        {
            ...
            using(var popup = ImRaii.PopupModal("###settings_edit_source", ref isItemEditorOpen, ImGuiWindowFlags.NoResize | ImGuiWindowFlags.NoMove | ImGuiWindowFlags.NoTitleBar))
            {
                ...
                if(selectedItem is FileSourceConfig fileSource)
                {
                    ...
                    ImGui.SetCursorPosY(ImGui.GetCursorPosY() + ImStarfallStudio.GetRemainingHeight() - ImStarfallStudio.GetLineHeight());

                    if(isNewItem)
                    {
                        ImGui.SetCursorPosX(ImGui.GetCursorPosX() + ImStarfallStudio.GetRemainingWidth() - 210);
                    ...
                    {
                        ImGui.SetCursorPosX(ImGui.GetCursorPosX() + ImStarfallStudio.GetRemainingWidth() - 110);
    ...
    {
        float itemHeight = (ImStarfallStudio.GetLineHeight() * 2) + ImGui.GetStyle().ItemSpacing.Y;

        float itemTop = ImGui.GetCursorPosY();
        ImGui.Selectable($"###settings_source_{id}_selectable", ref isSelected, ImGuiSelectableFlags.None, new(ImStarfallStudio.GetRemainingWidth(), itemHeight));

        bool isHover = ImGui.IsItemHovered();

        ImGui.SetCursorPosY(itemTop + ImGui.GetStyle().FramePadding.Y);

        ImStarfallStudio.FontIcon(sourceConfig.Enabled ? FontAwesomeIcon.Check : FontAwesomeIcon.Times);
        ImGui.SameLine();
    ...
    private static void DrawSource(FileSourceConfig config)
    {
        if(config.Root is not null and not Environment.SpecialFolder.MyComputer)
        {
            ...
            if(!valid)
            {
                ImStarfallStudio.FontIcon(FontAwesomeIcon.ExclamationTriangle);
                ImGui.SameLine();
