...
public static class AppearanceEditorCommon
{
    ...
    public static void DrawPenumbraCollectionSwitcher(ActorAppearanceCapability capability)
    {
        ...
        if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.EarthOceania))
        {
            capability.PenumbraService.OpenPenumbra();
        }

        ImStarfallStudio.AttachToolTip("Open Penumbra");
        ImGui.SameLine();
        ...
        ImStarfallStudio.AttachToolTip("Apply Penumbra Collection.");

        ImGui.SameLine();

        if(ImStarfallStudio.FontIconButtonRight("actorappearancewidget_reset", FontAwesomeIcon.Undo, 1, "Reset Collection", capability.IsCollectionOverridden))
    ...
    public static void DrawGlamourerDesignSwitcher(ActorAppearanceCapability capability)
    {
        ...
        if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.TheaterMasks))
        {
            capability.GlamourerService.OpenGlamourer();
        }

        ImStarfallStudio.AttachToolTip("Open Glamourer");
        ImGui.SameLine();
        ...
        if(capability.HasMCDF)
            ImStarfallStudio.AttachToolTip("Unable to apply Glamourer Design while a MCDF is applied!");
        else
            ImStarfallStudio.AttachToolTip("Apply Glamourer Design.");

        ImGui.SameLine();

        using(ImRaii.Disabled(capability.HasMCDF))
            if(ImStarfallStudio.FontIconButtonRight("actorappearancewidget_DesignReset", FontAwesomeIcon.Undo, 1, "Reset Design"))
    ...
    public static void DrawCustomizePlusProfileSwitcher(ActorAppearanceCapability capability)
    {
        ...
        ImStarfallStudio.AttachToolTip("Open Customize+");
        ImGui.SameLine();
        ...
        if(capability.HasMCDF)
            ImStarfallStudio.AttachToolTip("Unable to apply Customize+ profile while a MCDF is applied!");
        else
            ImStarfallStudio.AttachToolTip("Apply Customize+ profile.");

        ImGui.SameLine();

        using(ImRaii.Disabled(capability.HasMCDF))
            if(ImStarfallStudio.FontIconButtonRight("actorappearancewidget_ProfileReset", FontAwesomeIcon.Undo, 1, "Reset C+ Profile"))
                capability.ResetProfile();
    }
