...
public class PosingTransformEditor
{
    ...
    public void Draw(string id, PosingCapability posingCapability, bool compactMode = false)
    {
        ...
        using(ImRaii.PushId(id))
        {
            ...
            if(posingCapability.Actor.IsProp == false)
            {
                ...
                if(ImStarfallStudio.FontIconButton("transformOffset", FontAwesomeIcon.GaugeSimpleHigh, "Transform Movement Speed"))
                {
                    ImGui.OpenPopup("transformOffset");
                }
                ImStarfallStudio.AttachToolTip("Adjusts the speed of the transform controls");
                ...
                using(ImRaii.Disabled(isBone == false))
                {
                    ...
                    if(ImStarfallStudio.FontIconButton("propagate", FontAwesomeIcon.Compress, "Propagate", realBone?.EligibleForIK == true))
                    ...
                    if(compactMode)
                    {
                        ...
                        {
                            if(ImStarfallStudio.FontIconButton("clear_selection", FontAwesomeIcon.MinusSquare, "Clear Selection"))
                        ...
                        {
                            if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.LevelUpAlt))
                                posingCapability.Selected = new BonePoseInfoId(parentBone!.Name, parentBone!.PartialId, PoseInfoSlot.Character);
                        }
                        ImStarfallStudio.AttachToolTip("Select Parent");
                    }
                }
                ImGui.SameLine();

                using(ImRaii.Disabled(selectedIsBone.HasValue)) // This is borken to all hell
                    if(ImStarfallStudio.FontIconButton("copypaste", FontAwesomeIcon.Clipboard, "Copy & Paste Transform"))
                        ImGui.OpenPopup("CopyPastePopup");
                if(selectedIsBone.HasValue)
                    ImStarfallStudio.AttachToolTip("Copy & Paste is currently only available for Model Transform");

                using(ImRaii.Disabled(!posingCapability.CanResetBone(realBone)))
                {
                    ImGui.SameLine();

                    if(ImStarfallStudio.FontIconButtonRight("resetTransform", FontAwesomeIcon.Retweet, 1, tooltip: "Reset Bone"))
                    {
    ...
    private void DrawBoneTransformEditor(PosingCapability posingCapability, BonePoseInfoId boneId, bool compactMode = false)
    {
        ...
        using(ImRaii.Disabled(bone != null && bone.Freeze))
        {
            ...
            (var pdidChange, var panyActive) = ImStarfallStudio.DragFloat3($"###_transformPosition_0", ref realTransform.Position, offset, FontAwesomeIcon.ArrowsUpDownLeftRight, "Position", enableExpanded: compactMode);
            ImStarfallStudio.VerticalPadding(2);
            (var rdidChange, var ranyActive) = ImStarfallStudio.DragFloat3($"###_transformRotation_0", ref realEuler, offset * 100, FontAwesomeIcon.ArrowsSpin, "Rotation", enableExpanded: compactMode);
            ImStarfallStudio.VerticalPadding(2);
            (var sdidChange, var sanyActive) = ImStarfallStudio.DragFloat3($"###_transformScale_0", ref realTransform.Scale, offset, FontAwesomeIcon.ExpandAlt, "Scale", enableExpanded: compactMode);
            ImStarfallStudio.VerticalPadding(2);
    ...
    private void DrawModelTransformEditor(PosingCapability posingCapability, bool compactMode = false)
    {
        ...
        if(!StarfallStudio.TryGetService(out EntityManager entityManager))
        {
    ...
    private void DrawModelTransformEditorSingle(PosingCapability posingCapability, bool compactMode = false)
    {
        ...
        using(ImRaii.Disabled(posingCapability.ModelPosing.Freeze == true))
        {
            ...
            (var pdidChange, var panyActive) = ImStarfallStudio.DragFloat3($"###_transformPosition_1", ref realTransform.Position, offset, FontAwesomeIcon.ArrowsUpDownLeftRight, "Position", enableExpanded: compactMode);
            ImStarfallStudio.VerticalPadding(2);
            (var rdidChange, var ranyActive) = ImStarfallStudio.DragFloat3($"###_transformRotation_1", ref realEuler, offset * 100, FontAwesomeIcon.ArrowsSpin, "Rotation", enableExpanded: compactMode);
            ImStarfallStudio.VerticalPadding(2);
            ...
            if(isProp)
            {
                ImStarfallStudio.Icon(FontAwesomeIcon.ExpandAlt);

                ImGui.SameLine();

                Vector2 size = new(0, 0)
                {
                    X = ImStarfallStudio.GetRemainingWidth() + ImGui.GetStyle().ItemSpacing.X
                };

                float entryWidth = (size.X - (ImGui.GetStyle().ItemSpacing.X * 2));
                ImGui.SetNextItemWidth(entryWidth);

                (sanyActive, sdidChange) = ImStarfallStudio.DragFloat($"##transformScale", ref realTransform.Scale.X, offset / 10);
            }
            else
                (sdidChange, sanyActive) = ImStarfallStudio.DragFloat3($"###_transformScale_1", ref realTransform.Scale, offset, FontAwesomeIcon.ExpandAlt, "Scale", enableExpanded: compactMode);
    ...
    private void DrawModelTransformEditorMulti(List<(ActorEntity actor, PosingCapability capability, Transform transform)> selectedActors, PosingCapability primaryCapability, bool compactMode = false)
    {
        ...
        using(ImRaii.Disabled(anyFrozen))
        {
            ...
            (var pdidChange, var panyActive) = ImStarfallStudio.DragFloat3($"###_transformPosition_1", ref primaryTransform.Position, offset, FontAwesomeIcon.ArrowsUpDownLeftRight, "Position (Group)", enableExpanded: compactMode);
            ImStarfallStudio.VerticalPadding(2);
            (var rdidChange, var ranyActive) = ImStarfallStudio.DragFloat3($"###_transformRotation_1", ref realEuler, offset * 100, FontAwesomeIcon.ArrowsSpin, "Rotation (Pivot)", enableExpanded: compactMode);
            ImStarfallStudio.VerticalPadding(2);
            (var sdidChange, var sanyActive) = ImStarfallStudio.DragFloat3($"###_transformScale_1", ref primaryTransform.Scale, offset, FontAwesomeIcon.ExpandAlt, "Scale (Group)", enableExpanded: compactMode);
            ImStarfallStudio.VerticalPadding(2);
            ...
            if(didChange)
            {
                if(_groupedPendingSnapshot == null && 
                    StarfallStudio.TryGetService<HistoryService>(out HistoryService? historyService) && 
                    StarfallStudio.TryGetService<EntityManager>(out EntityManager? entityManager))
                {
            ...
            else
            {
                if(_trackingEuler.HasValue || _trackingTransform.HasValue)
                {
                    ...
                    {
                        if(StarfallStudio.TryGetService<HistoryService>(out HistoryService? historyService))
                        {
        ...
        if(popup.Success)
        {
            ...
            if(selectedIsBone.HasValue && !posingCapability.Actor.IsProp)
            {
                ...
                if(bone is not null)
                {
                    ...
                    ImStarfallStudio.DragFloat($"##transformSpeed_1", ref bone.BoneAdjustmentOffset, 0.001f, 10f, 0.01f, "Offset", 50);
            ...
            {
                ImStarfallStudio.DragFloat($"##transformSpeed_1", ref posingCapability.ModelPosing.TransformOffset, 0.001f, 10f, 0.01f, "Offset", 50);
