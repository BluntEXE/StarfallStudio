...
public static unsafe partial class WindowsCloudAPI
{
    ...
    public static bool IsCloudSynced(string path)
    {
        ...
        {
            StarfallStudio.Log.Debug($"{nameof(CfGetSyncRootInfoByPath)} threw DllNotFoundException");
            return false;
        }
        catch(EntryPointNotFoundException)
        {
            StarfallStudio.Log.Debug($"{nameof(CfGetSyncRootInfoByPath)} threw EntryPointNotFoundException");
            return false;
        }

        StarfallStudio.Log.Debug($"{nameof(CfGetSyncRootInfoByPath)} returned HRESULT 0x{hresult:X8}");
        if(hresult < 0)
        {
            return false;
        }
        ...
        {
            StarfallStudio.Log.Debug($"Expected {nameof(CfGetSyncRootInfoByPath)} to return {sizeof(long)} bytes, got {length} bytes");
            return false;
        }

        StarfallStudio.Log.Debug($"{nameof(CfGetSyncRootInfoByPath)} returned {{ SyncRootFileId = 0x{*buffer:X16} }}");
