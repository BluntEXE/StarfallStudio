...
public struct DiagnosticTrace()
{
    ...
    public readonly void Log(string label = "Trace")
        => StarfallStudio.Log.Verbose($"[Diagnostics]:[{label}] Samples:[{SampleCount}] | avg={AvgMs:F3}ms min={MinMs:F3}ms max={MaxMs:F3}ms | avgdata={AvgCustomData:F1}");
...
public struct DiagnosticTracker(string tag, int logInterval, double slowFrameThresholdMs = 100, int slowFrameCooldownFrames = 5000, Dictionary<string, DiagnosticTrace>? diagnosticTraces = null)
{
    ...
    /// <summary>
    /// Outputs the trace's to StarfallStudio.Log, then resets them.
    /// </summary>
    public void Log()
    {
        Trace.Log(Tag);
        ...
        foreach(var tracesKVP in _dynamicTraces)
        {
            ...
            StarfallStudio.Log.Verbose($"  [Diagnostics]:[{tracesKVP.Key}] avg={trace.AvgMs:F3}ms max={trace.MaxMs:F3}ms");
            trace.Reset();
        }
    }
}
