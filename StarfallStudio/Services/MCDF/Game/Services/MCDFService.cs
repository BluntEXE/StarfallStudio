...
using StarfallStudio.MCDF.API.Data;
using StarfallStudio.MCDF.Game.FileCache;
...
namespace StarfallStudio.MCDF.Game.Services;
...
public class MCDFService : IDisposable
{
    ...
    private void OnGPoseStateChange(bool newState)
    {
        if(newState is false)
        {
            ...
            {
                StarfallStudio.Log.Error(ex, "Exception while trying to stop the application process of a MCDF on GPose exit");
            }
        }
    }
        ...
        Dictionary<string, string> modPaths, string? manipData, string? glamourerData, string? customizeData, CancellationToken token, ILogger logger)
    {
        ...
        try
        {
            ...
            penumbraCollection = await _penumbraService.CreateTemporaryCollectionAsync($"StarfallStudio_{idx}").ConfigureAwait(false);
    ...
    internal async Task SaveCharaFileAsync(string description, string filePath, IGameObject gameObject)
    {
        ...
        try
        {
            StarfallStudio.Log.Info("Starting MCDF export...");
            ...
            await Task.Run(async () =>
            {
                ...
                StarfallStudio.Log.Info("MCDF export complete!");
            });
        }
        catch(Exception ex)
        {
            StarfallStudio.Log.Warning(ex, "Failure Saving Mare Chara File, deleting output");
    ...
    public async Task<CharacterDataFragment?> BuildCharacterData(IGameObject playerRelatedObject, CancellationToken token)
    {
        ...
        try
        {
            ...
            {
                pointerIsZero = true;
                StarfallStudio.Log.Debug("NullRef for {object}", playerRelatedObject);
            }
        }
        catch(Exception ex)
        {
            StarfallStudio.Log.Warning(ex, "Could not create data for {object}", playerRelatedObject);
        }

        if(pointerIsZero)
        {
            StarfallStudio.Log.Debug("Pointer was zero for {objectKind}", playerRelatedObject.ObjectKind);
            return null;
        }
        ...
        {
            StarfallStudio.Log.Debug("Cancelled creating Character data for {object}", playerRelatedObject);
            throw;
        }
        catch(Exception e)
        {
            StarfallStudio.Log.Warning(e, "Failed to create {object} data", playerRelatedObject);
        }
    ...
    private async Task<CharacterDataFragment> CreateCharacterData(IGameObject playerRelatedObject, CancellationToken ct)
    {
        ...
        StarfallStudio.Log.Verbose("Building character data for {obj}", playerRelatedObject);
        ...
        {
            StarfallStudio.Log.Debug("Character is null but it shouldn't be, waiting");
            await Task.Delay(50, ct).ConfigureAwait(false);
            totalWaitTime -= 50;
        }

        // Make sure StarfallStudio can't MCDF if the actor had a "Mare" actor loaded on it by cheaking for lock from glamourer
        if(_glamourerService.CheckForLock(playerRelatedObject))
        {
            StarfallStudio.Log.Information("Unable to apply MCDF, Actor is Locked by Glamourer");
            StarfallStudio.NotifyError("Unable to apply MCDF! Actor is Locked! Are you using a sync service?!");
        ...
        StarfallStudio.Log.Verbose("== Static Replacements ==");
        ...
        {
            StarfallStudio.Log.Verbose("=> {repl}", replacement);
            ct.ThrowIfCancellationRequested();
        }
        ...
        StarfallStudio.Log.Verbose("Handling transient update for {obj}", playerRelatedObject);
        ...
        StarfallStudio.Log.Verbose("== Transient Replacements ==");
        ...
        StarfallStudio.Log.Verbose("Glamourer is now: {data}", fragment.GlamourerString);

        fragment.CustomizePlusScale = await getCustomizeData.ConfigureAwait(false) ?? string.Empty;
        StarfallStudio.Log.Verbose("Customize is now: {data}", fragment.CustomizePlusScale);
        ...
        if(objectKind == ObjectKind.Pc)
        {
            ...
            //    StarfallStudio.Log.Verbose("Honorific is now: {data}", playerFragment!.HonorificData);

            //    playerFragment!.HeelsData = await getHeelsOffset.ConfigureAwait(false);
            //    StarfallStudio.Log.Verbose("Heels is now: {heels}", playerFragment!.HeelsData);
            ...
            //    StarfallStudio.Log.Verbose("Moodles is now: {moodles}", playerFragment!.MoodlesData);

            //    playerFragment!.PetNamesData = _ipcManager.PetNames.GetLocalNames();
            //    StarfallStudio.Log.Verbose("Pet Nicknames is now: {petnames}", playerFragment!.PetNamesData);
        }

        ct.ThrowIfCancellationRequested();

        var toCompute = fragment.FileReplacements.Where(f => !f.IsFileSwap).ToArray();
        StarfallStudio.Log.Verbose("Getting Hashes for {amount} Files", toCompute.Length);
        ...
        if(removed > 0)
        {
            StarfallStudio.Log.Verbose("Removed {amount} of invalid files", removed);
        }
        ...
        if(objectKind == ObjectKind.Pc)
        {
            ...
            {
                StarfallStudio.Log.Debug(e, "Cancelled during player animation verification");
                throw;
            }
            catch(Exception e)
            {
                StarfallStudio.Log.Warning(e, "Failed to verify player animations, continuing without further verification");
            }
        }

        StarfallStudio.Log.Info("Building character data for {obj} took {time}ms", objectKind, TimeSpan.FromTicks(DateTime.UtcNow.Ticks - start.Ticks).TotalMilliseconds);
    ...
    private async Task VerifyPlayerAnimationBones(Dictionary<string, List<ushort>>? boneIndices, CharacterDataFragmentPlayer fragment, CancellationToken ct)
    {
        ...
        {
            StarfallStudio.Log.Verbose("Found {skellyname} ({idx} bone indices) on player: {bones}", kvp.Key, kvp.Value.Any() ? kvp.Value.Max() : 0, string.Join(',', kvp.Value));
        }
        ...
        foreach(var file in fragment.FileReplacements.Where(f => !f.IsFileSwap && f.GamePaths.First().EndsWith("pap", StringComparison.OrdinalIgnoreCase)).ToList())
        {
            ...
            if(skeletonIndices != null)
            {
                ...
                {
                    StarfallStudio.Log.Verbose("All indices of {path} are <= 105, ignoring", file.ResolvedPath);
                    continue;
                }

                StarfallStudio.Log.Verbose("Verifying bone indices for {path}, found {x} skeletons", file.ResolvedPath, skeletonIndices.Count);

                foreach(var boneCount in skeletonIndices.Select(k => k).ToList())
                {
                    if(boneCount.Value.Max() > boneIndices.SelectMany(b => b.Value).Max())
                    {
                        StarfallStudio.Log.Debug("Found more bone indices on the animation {path} skeleton {skl} (max indice {idx}) than on any player related skeleton (max indice {idx2})",
            ...
            {
                noValidationFailed++;
                StarfallStudio.Log.Verbose("Removing {file} from sent file replacements and transient data", file.ResolvedPath);
    ...
    public unsafe Dictionary<string, List<ushort>>? GetSkeletonBoneIndices(IGameObject handler)
    {
        ...
        try
        {
            for(int i = 0; i < chara->Skeleton->PartialSkeletonCount; i++)
            {
                ...
                //StarfallStudio.Log.Verbose("Iterating over SkeletonResourceHandle #{i}:{x}", i, ((nint)handle).ToString("X"));
        ...
        {
            StarfallStudio.Log.Warning(ex, "Could not process skeleton data");
        }
    ...
    public unsafe Dictionary<string, List<ushort>>? GetBoneIndicesFromPap(string hash)
    {
        ...
        {
            StarfallStudio.Log.Warning(ex, "Could not load havok file in {path}", tempHavokDataPath);
        }
        finally
        {
