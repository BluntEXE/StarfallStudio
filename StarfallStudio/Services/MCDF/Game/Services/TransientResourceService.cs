...
using StarfallStudio.MCDF.API.Data.Enum;
using StarfallStudio.MCDF.Utils;
...
public class TransientResourceService : IDisposable
{
    ...
    public void CleanUpSemiTransientResources(ObjectKind objectKind, List<FileReplacement>? fileReplacement = null)
    {
        ...
        if(removedPaths > 0)
        {
            StarfallStudio.Log.Verbose("Removed {amount} of SemiTransient paths during CleanUp, Saving from {name}", removedPaths, nameof(CleanUpSemiTransientResources));
    ...
    public void PersistTransientResources(ObjectKind objectKind)
    {
        ...
        StarfallStudio.Log.Verbose("Persisting {count} transient resources", transientResources.Count);
        ...
        if(saveConfig)
        {
            StarfallStudio.Log.Verbose("Saving transient.json from {method}", nameof(PersistTransientResources));
            _configurationService.Save();
        }
    ...
    public void RemoveTransientResource(ObjectKind objectKind, string path)
    {
        if(SemiTransientResources.TryGetValue(objectKind, out var resources))
        {
            ...
            if(objectKind == ObjectKind.Player)
            {
                ...
                StarfallStudio.Log.Verbose("Saving transient.json from {method}", nameof(RemoveTransientResource));
                _configurationService.Save();
            }
        }
    }
    ...
    internal void ClearTransientPaths(ObjectKind objectKind, List<string> list)
    {
        ...
        {
            StarfallStudio.Log.Verbose("Ignored {0} game paths when clearing transients", recordingOnlyRemoved);
        }
        ...
        if(TransientResources.TryGetValue(objectKind, out var set))
        {
            ...
            {
                StarfallStudio.Log.Verbose("Removing From Transient: {file}", file);
            }

            int removed = set.RemoveWhere(p => list.Contains(p, StringComparer.OrdinalIgnoreCase));
            StarfallStudio.Log.Verbose("Removed {removed} previously existing transient paths", removed);
        }
        ...
        if(objectKind == ObjectKind.Player && SemiTransientResources.TryGetValue(objectKind, out var semiset))
        {
            ...
            {
                StarfallStudio.Log.Verbose("Removing From SemiTransient: {file}", file);
            ...
            StarfallStudio.Log.Verbose("Removed {removed} previously existing semi transient paths", removed);
            if(removed > 0)
            {
                reloadSemiTransient = true;
                StarfallStudio.Log.Verbose("Saving transient.json from {method}", nameof(ClearTransientPaths));
                _configurationService.Save();
            }
        }
    ...
    private void DalamudUtil_FrameworkUpdate()
    {
        ...
        foreach(var kind in Enum.GetValues(typeof(ObjectKind)))
        {
            ...
            {
                StarfallStudio.Log.Verbose("Object not present anymore: {kind}", kind);
            }
        }
    }
    ...
    private void Manager_PenumbraResourceLoadEvent(IntPtr GameObject, string GamePath, string FilePath)
    {
        ...
        if(transientContains || semiTransientContains)
        {
            if(!IsTransientRecording)
                StarfallStudio.Log.Verbose("Not adding {replacedPath} => {filePath}, Reason: Transient: {contains}, SemiTransient: {contains2}", replacedGamePath, filePath,
        ...
        else
        {
            if(!IsTransientRecording)
            {
                ...
                if(isAdded)
                {
                    StarfallStudio.Log.Verbose("Adding {replacedGamePath} for {gameObject} ({filePath})", replacedGamePath, owner?.ToString() ?? gameObjectAddress.ToString("X"), filePath);
    ...
    private void SendTransients(nint gameObject, ObjectKind objectKind)
    {
        _ = Task.Run(async () =>
        {
            ...
            foreach(var kvp in TransientResources)
            {
                ...
                {
                    StarfallStudio.Log.Verbose("Sending Transients for {kind}", objectKind);
    ...
    internal void SaveRecording()
    {
        ...
        //    StarfallStudio.Log.Verbose("Adding recorded: {gamePath} => {filePath}", item.GamePath, item.FilePath);
