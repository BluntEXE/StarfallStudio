...
public class DirectoryEntry : GroupEntryBase
{
    ...
    public override void DrawActions(bool isModal)
    {
        ...
        {
            if(ImStarfallStudio.FontIconButton(FontAwesomeIcon.FolderOpen))
            {
...
public class FileEntry : ItemEntryBase
{
    ...
    public void LoadNewPreviewImage(string filePath)
    {
        ...
        {
            StarfallStudio.Log.Error(ex, "Failed to load new preview image.");
        }
    }
    ...
    private void EditDetails(EditDetailsDelegate handler)
    {
        ...
        {
            StarfallStudio.Log.Error(ex, "Exception while trying to modify a pose file!");
        }
    }
    ...
    public override void EditDetailsPopup(bool openPopup)
    {
        ...
        {
            ImGui.SameLine();
            ImStarfallStudio.HorizontalPadding(10);
            ...
            ImStarfallStudio.ImageFit(_editPreviewImage, new Vector2(500, scaledHeight));
        }

        ImStarfallStudio.VerticalPadding(ImGui.GetTextLineHeight() / 2);
        float buttonsWidth = ImGui.CalcTextSize("Save").X + ImGui.CalcTextSize("Cancel").X + ImGui.GetStyle().FramePadding.X * 4f
            + ImGui.GetStyle().ItemSpacing.X;
        ImStarfallStudio.RightAlign(buttonsWidth);
