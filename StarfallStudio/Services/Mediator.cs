...
public class Mediator() : IDisposable
{
    ...
    {
        StarfallStudio.Log.Info("=== Mediator Subscriber Info ===");
        ...
        {
            StarfallStudio.Log.Info("Subscriber {type}: {sub}", subscriber.GetType().Name, subscriber?.ToString() ?? "Unknown Subscriber");
            ...
            if(!string.Equals(sb.ToString(), "=> ", StringComparison.Ordinal))
                StarfallStudio.Log.Info("{sb}", sb.ToString().TrimEnd(' ', ','));

            StarfallStudio.Log.Info("---");
        }
    }

    public Task StartAsync(CancellationToken cancellationToken)
    {
        StarfallStudio.Log.Info("Starting Mediator");
        ...
        StarfallStudio.Log.Info("Started Mediator");
    ...
    internal void UnsubscribeAll(IMediatorSubscriber subscriber)
    {
        lock(_addRemoveLock)
        {
            foreach(Type kvp in _subscriberDict.Select(k => k.Key))
            {
                ...
                if(unSubbed > 0)
                {
                    StarfallStudio.Log.Debug("{sub} unsubscribed from {msg}", subscriber.GetType().Name, kvp.Name);
                }
            }
        }
    }
    ...
    private void ExecuteReflected<T>(List<SubscriberAction> subscribers, T message) where T : MessageBase
    {
        foreach(SubscriberAction subscriber in subscribers)
        {
            ...
            catch(Exception ex)
            {
                ...
                StarfallStudio.Log.Error("Error executing {type} for subscriber {subscriber}: {ex}",
