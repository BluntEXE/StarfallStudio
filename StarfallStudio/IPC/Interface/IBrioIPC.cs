...
public interface IStarfallStudioIPC : IDisposable
{
    string Name { get; }
...
public abstract class StarfallStudioIPC : IStarfallStudioIPC
{
    ...
    public IPCStatus CheckStatus(bool force = false)
    {
        ...
        try
        {
            ...
            if(!installed)
            {
                StarfallStudio.Log.Verbose($"{Name} not present");
            ...
            {
                StarfallStudio.Log.Debug($"{Name} API Version mismatch, found v{major}.{minor}");
        ...
        {
            StarfallStudio.Log.Debug(ex, $"{Name} initialize error");
