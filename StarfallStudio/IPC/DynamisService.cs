...
// - Integrated code so it can be used for StarfallStudioIPC
// - Changed logging to use StarfallStudio.Log
...
public class DynamisService : StarfallStudioIPC
{
    ...
    private void OnInitialized(uint major, uint minor, ulong flags, Version _)
    {
        ...
        if(major is not 1)
        {
            StarfallStudio.Log.Debug($"Could not attach to Dynamis {VersionMajor}.{VersionMinor}, only 1.X is supported.");
            return;
        }

        if(minor < 6)
        {
            StarfallStudio.Log.Debug($"Could not attach to Dynamis {VersionMajor}.{VersionMinor}, only 1.6 or higher is supported.");
            return;
        }
        ...
        try
        {
            ...
            StarfallStudio.Log.Debug($"Attached to Dynamis {VersionMajor}.{VersionMinor}.");
        }
        catch(Exception ex)
        {
            Error = ex;
            StarfallStudio.Log.Error($"Error subscribing to Dynamis IPC:\n{ex}");
            OnDisposed();
        }
    }
    ...
    {
        if(IsAttached)
            StarfallStudio.Log.Information($"Detaching from Dynamis! {VersionMajor}.{VersionMinor}.");
