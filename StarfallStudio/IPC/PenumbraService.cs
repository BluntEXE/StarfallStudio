...
namespace StarfallStudio.IPC;

public class PenumbraService : StarfallStudioIPC
{
    ...
    public Guid? SetCollectionForObject(IGameObject gameObject, Guid collectionName)
    {
        ...
        StarfallStudio.Log.Debug($"Setting GameObject {gameObject.ObjectIndex} collection to {collectionName}");
    ...
    public async Task<Dictionary<string, HashSet<string>>?> GetCharacterData(IGameObject gameObject)
    {
        ...
        {
            StarfallStudio.Log.Debug("Calling On IPC: Penumbra.GetGameObjectResourcePaths");
    ...
    public string GetMetaManipulations()
    {
        ...
        StarfallStudio.Log.Debug("Calling On IPC: Penumbra.GetMetaManipulations");
    ...
    public async Task RemoveTemporaryCollectionAsync(Guid applicationId, Guid collId)
    {
        ...
        {
            StarfallStudio.Log.Debug("[{applicationId}] Removing temp collection for {collId}", applicationId, collId);
            var ret2 = _penumbraRemoveTemporaryCollection.Invoke(collId);
            StarfallStudio.Log.Debug("[{applicationId}] RemoveTemporaryCollection: {ret2}", applicationId, ret2);
        }).ConfigureAwait(false);
    }
    ...
    public async Task<Guid> CreateTemporaryCollectionAsync(string uid)
    {
        ...
        return await _framework.RunOnFrameworkThread(() =>
        {
            var collName = "StarfallStudio_" + uid;
            _penumbraCreateNamedTemporaryCollection.Invoke("StarfallStudio", collName, out var collId);
            StarfallStudio.Log.Debug("Creating Temp Collection {collName}, GUID: {collId}", collName, collId);
            return collId;
    ...
    public async Task SetTemporaryModsAsync(Guid applicationId, Guid collId, Dictionary<string, string> modPaths)
    {
        ...
        await _framework.RunOnFrameworkThread(() =>
        {
            ...
            {
                StarfallStudio.Log.Debug("[{applicationId}] Change: {from} => {to}", applicationId, mod.Key, mod.Value);
            }
            var retRemove = _penumbraRemoveTemporaryMod.Invoke("StarfallStudioChara_Files", collId, 0);
            StarfallStudio.Log.Debug("[{applicationId}] Removing temp files mod for {collId}, Success: {ret}", applicationId, collId, retRemove);
            var retAdd = _penumbraAddTemporaryMod.Invoke("StarfallStudioChara_Files", collId, modPaths, string.Empty, 0);
            StarfallStudio.Log.Debug("[{applicationId}] Setting temp files mod for {collId}, Success: {ret}", applicationId, collId, retAdd);
        }).ConfigureAwait(false);
    }
    ...
    public async Task SetManipulationDataAsync(Guid applicationId, Guid collId, string manipulationData)
    {
        ...
        await _framework.RunOnFrameworkThread(() =>
        {
            StarfallStudio.Log.Debug("[{applicationId}] Manip: {data}", applicationId, manipulationData);
            var retAdd = _penumbraAddTemporaryMod.Invoke("StarfallStudioChara_Meta", collId, [], manipulationData, 0);
            StarfallStudio.Log.Debug("[{applicationId}] Setting temp meta mod for {collId}, Success: {ret}", applicationId, collId, retAdd);
        }).ConfigureAwait(false);
    }
