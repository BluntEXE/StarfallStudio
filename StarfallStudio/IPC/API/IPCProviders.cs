...
public class IPCProviders : MediatorSubscriberBase
{
    ...
    private readonly StarfallStudioEventProvider _deinitializedProvider;
    private readonly StarfallStudioEventProvider _initializedProvider;

    public readonly StarfallStudioEventProvider<IGameObject> ActorDespawned;
    public readonly StarfallStudioEventProvider<IGameObject> ActorSpawned;

    public IPCProviders(Mediator mediator, IDalamudPluginInterface pi, StarfallStudioAPIService brioAPI) : base(mediator)
    {
        _deinitializedProvider = Deinitialized.Provider(pi);
        _initializedProvider = Initialized.Provider(pi);

        ActorDespawned = global::StarfallStudio.API.ActorDestroyed.Provider(pi);
        ActorSpawned = global::StarfallStudio.API.ActorSpawned.Provider(pi);
