...
public class GlamourerService : StarfallStudioIPC
{
    ...
    public bool CheckForLock(IGameObject? character)
    {
        ...
        StarfallStudio.Log.Verbose("Glamourer CheckForLock... " + key);
    ...
    public bool UnlockAndRevertCharacterByName(string name)
    {
        ...
        StarfallStudio.Log.Debug("Starting glamourer UnlockAndRevertByName...");

        var success = _glamourerUnlockByName.Invoke(name, LockCode);

        if(success is not GlamourerApiEc.Success)
        {
            StarfallStudio.Log.Info($"Glamourer UnlockAndRevertCharacterByName was not Successful: {success}");
            return false;
        }
    ...
    public bool UnlockAndRevertCharacter(IGameObject? character)
    {
        ...
        StarfallStudio.Log.Debug("Starting glamourer UnlockAndRevert...");

        var success = _glamourerRevertCharacter.Invoke(character!.ObjectIndex, LockCode);

        if(success is not GlamourerApiEc.Success)
        {
            StarfallStudio.Log.Info($"Glamourer UnlockAndRevertCharacter was not Successful: {success}");
            return false;
        }
    ...
    public Task RevertCharacter(IGameObject? character)
    {
        ...
        StarfallStudio.Log.Debug("Starting glamourer Revert...");
        ...
        {
            StarfallStudio.Log.Info("Glamourer character was locked..");
        ...
        {
            StarfallStudio.Log.Debug("Glamourer revert Started");
            return _framework.RunOnTick(async () =>
            {
                await _redrawService.WaitForDrawing(character!);
                StarfallStudio.Log.Debug("Glamourer revert complete");
            }, delayTicks: 5);
        }
    ...
    public void RevertByName(string name, Guid applicationId)
    {
        ...
        try
        {
            StarfallStudio.Log.Debug("[{appid}] Calling On IPC: GlamourerRevertByName", applicationId);
            _glamourerRevertByName.Invoke(name, LockCode);
            StarfallStudio.Log.Debug("[{appid}] Calling On IPC: GlamourerUnlockName", applicationId);
            _glamourerUnlockByName.Invoke(name, LockCode);
        }
        catch(Exception ex)
        {
            StarfallStudio.Log.Warning(ex, "Error during Glamourer RevertByName");
        }
    }
