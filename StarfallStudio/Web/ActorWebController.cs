...
using StarfallStudioTransform = StarfallStudio.Core.Transform;

namespace StarfallStudio.Web;
...
public class ActorWebController(IFramework framework, PosingAPI posingAPI, SkeletonService skeletonService, ActorSpawnService actorSpawnService, ActorRedrawService redrawService) : WebApiController
{
    ...
    {
        StarfallStudio.Log.Debug("Received redraw request on WebAPI");
        try
        {
    ...
    {
        StarfallStudio.Log.Debug("Received spawn request on WebAPI");
        try
        {
    ...
    {
        StarfallStudio.Log.Debug("Received despawn request on WebAPI");
        try
        {
    ...
    public void GetStatus()
    {
        ...
        {
            status = "online",
            version = $"{StarfallStudio.MajorAPIVersion}.{StarfallStudio.MinorAPIVersion}",
            brioAvailable = true,
    ...
    public void Bones([JsonData] Dictionary<string, BoneTransformDTO> data)
    {
        ...
        var boneTransforms = new Dictionary<string, StarfallStudioTransform>();

        StarfallStudio.Log.Debug($"Received bone transform update for object ID {objectId} with {data.Count} bones.");
        foreach(var kvp in data)
        {
            boneTransforms[kvp.Key] = new StarfallStudioTransform
            {
            ...
            StarfallStudio.Log.Debug($"Bone: {kvp.Key}, Pos: {boneTransforms[kvp.Key].Position}, Rot: {boneTransforms[kvp.Key].Rotation}, Scale: {boneTransforms[kvp.Key].Scale}");
        }
...
public class PoseableCharacter
{
    ...
    /// <summary>
    /// Raw pose data in JSON format (compatible with StarfallStudio/Anamnesis)
    /// </summary>
