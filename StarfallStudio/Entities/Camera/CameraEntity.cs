...
public class CameraEntity(IServiceProvider provider, int cameraID, CameraType cameraType) : Entity(new EntityId(new CameraId(cameraID)), provider)
{
    ...
    public override void OnAttached()
    {
        ...
        AddCapability(ActivatorUtilities.CreateInstance<StarfallStudioCameraCapability>(_serviceProvider, this));
    }
        ...
        using(ImRaii.Disabled(_gPoseService.IsGPosing == false))
        {
            ...
            if(VirtualCamera.IsFreeCamera)
            {
                ...
                using(ImRaii.PushColor(ImGuiCol.Button, 0))
                    if(ImStarfallStudio.ToggelFontIconButton($"###{Id}_camera_movement", FontAwesomeIcon.Walking, new System.Numerics.Vector2(0), VirtualCamera.FreeCamValues.IsMovementEnabled, hoverText: toolTip1))
                    {
            ...
            {
                if(ImStarfallStudio.FontIconButtonRight($"###{Id}_camera_contextButton", FontAwesomeIcon.LocationCrosshairs, 1f, toolTip, bordered: false))
                {
