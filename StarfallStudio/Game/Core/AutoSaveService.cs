...
public class AutoSaveService : IDisposable
{
    ...
    public void Stop()
    {
        ...
        StarfallStudio.Log.Verbose($"AutoSave: Stop");
    ...
    private void AutoSave()
    {
        ...
        if(ConfigurationService.Instance.Configuration.AutoSave.AutoSaveSystemEnabled)
        {
            ...
            {
                StarfallStudio.Log.Verbose($"IsApplyingMCDF aborting this autosave ");
                return;
            }
            ...
            try
            {
                ...
                StarfallStudio.Log.Verbose($"AutoSaving: {path}");
                ...
                StarfallStudio.Log.Verbose($"AutoSaved!");

                CleanOldSaves();
            }
            catch(Exception ex)
            {
                StarfallStudio.Log.Error(ex, "Exception AutoSaving!");
            }
        }
    }
            ...
            (success, paths) =>
            {
                if(success && paths.Count == 1)
                {
                    ...
                    if(path.IsNullOrEmpty() == false)
                    {
                        ...
                        {
                            StarfallStudio.NotifyError("StarfallStudio AutoSave save was corrupted!");
                            StarfallStudio.Log.Error(ex, "Exception while loading an AutoSave!");
                        }
                    }
                }
            },
    ...
    {
        StarfallStudio.Log.Verbose($"AutoSave: Update");
    ...
    public void CleanOldSaves()
    {
        ...
        {
            StarfallStudio.Log.Error(ex, "Exception While Cleaning old AutoSaves!");
        }
    }
    ...
    private void GPoseService_OnGPoseStateChange(bool newState)
    {
        if(newState)
        {
            StarfallStudio.Log.Verbose($"AutoSave: GPoseStateChange Start");
            Start();
        }
        else
        {
            StarfallStudio.Log.Verbose($"AutoSave: GPoseStateChange Stop");
            Stop();
        }
    }
