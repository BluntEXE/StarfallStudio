...
public unsafe class SkeletonService : IDisposable
{
    ...
    public Skeleton? GetSkeleton(StarfallStudioCharacterBase* charaBase)
    {
    ...
    private void ApplyStarfallStudioTransforms(Skeleton skeleton, SkeletonPosingCapability posingCapability)
    {
    ...
    private unsafe void ReparentAttachments(Skeleton skeleton)
    {
        ...
        if(attach->AttachmentCount > 0)
        {
            ...
            if(attachedPtr != null)
            {
                ...
                {
                    AttachType.CharacterBase => ((StarfallStudioCharacterBase*)attachedPtr)->CharacterBase.Skeleton,
    ...
    private void BeginSkeletonUpdate()
    {
        ...
        foreach(var skeleton in _skeletonsToUpdate)
        {
            ApplyStarfallStudioTransforms(skeleton, _skeletonToPosingCapability[skeleton]);
    ...
    {
        StarfallStudio.Log.Debug("Refreshing skeleton cache...");
        ...
        foreach(var actor in _monitorService.ObjectTable)
        {
            if(actor is ICharacter chara)
            {
                ...
                foreach(var charaBase in bases)
                {
                    ...
                    StarfallStudio.Log.Debug($"Skeleton cached {actor.Name}");
                }
            }
        }
        StarfallStudio.Log.Debug("Skeleton cache refreshed.");
    }
    ...
    private void ClearSkeleton(StarfallStudioCharacterBase* charaBase)
    {
    ...
    private void CacheSkeleton(StarfallStudioCharacterBase* charaBase)
    {
    ...
    private void OnCharacterBaseMaterialsUpdate(StarfallStudioCharacterBase* charaBase)
    {
        ...
        {
            StarfallStudio.Log.Error(e, "Error during skeleton caching");
        }
    }

    private void OnCharacterBaseCleanup(StarfallStudioCharacterBase* charaBase)
    {
        try
        {
            ClearSkeleton(charaBase);
        ...
        {
            StarfallStudio.Log.Error(e, "Error during skeleton cleanup");
        }
    }
    ...
    private nint UpdateBonePhysicsDetour(nint a1)
    {
        ...
        {
            StarfallStudio.Log.Error(e, "Error during skeleton update");
        }
        return result;
    }
    ...
    private void FinalizeSkeletonsHook(nint a1)
    {
        ...
        {
            StarfallStudio.Log.Error(e, "Error during skeleton finalization");
        }
    }
