...
using StarfallStudio.Services.MediatorMessages;
...
namespace StarfallStudio.Game.Actor;
...
public class ActorSpawnService : MediatorSubscriberBase
{
        ...
        if(CreateCharacter(out ICharacter? chara, SpawnFlags.IsProp | SpawnFlags.CopyPosition, true))
        {
            ...
            (__) =>
            {
                ...
                if(entity is not null)
                {
                    ...
                    {
                        var acf = JsonSerializer.Deserialize<AnamnesisCharaFile>(ResourceProvider.Instance.GetRawResourceString("Data.StarfallStudioPropChar.chara"));
                        entity.GetCapability<ActorAppearanceCapability>().SetAppearanceAsTask(acf, AppearanceImportOptions.Default);
                       
                        _framework.RunOnTick(() =>
                        {
                            entity.GetCapability<PosingCapability>().LoadResourcesPose("Data.StarfallStudioPropPose.pose");
    ...
    {
        StarfallStudio.Log.Debug("Destroying all created gameobjects.");
            ...
            if(obj is not null)
            {
                try
                {
                    ...
                    if(obj is not null)
                        ...
                        StarfallStudio.Log.Fatal($"CleanObject could not be called because the object was null idx:{idx}");
                }
                catch(Exception ex)
                {
                    StarfallStudio.Log.Warning(ex, $"Exception while destroying all the created objects idx:{idx}");
                }
            }
    ...
    public void CleanObject(IGameObject? go, bool disposing)
    {
        ...
        StarfallStudio.Log.Debug($"Destroying gameobject: {go.ObjectIndex}...");
    ...
    private bool CreateEmptyCharacter([MaybeNullWhen(false)] out ICharacter outCharacter, SpawnFlags flags)
    {
        ...
        StarfallStudio.Log.Debug("Creating StarfallStudio character...");
        ...
        unsafe
        {
            ...
            {
                StarfallStudio.Log.Warning("Failed to create character, invalid ID was returned.");
            ...
            ((StructsObject*)newObject)->CalculateAndSetName(newId); // StarfallStudio One etc
    ...
    public unsafe SpawnFlags GetSpawnFlagsByIndex(ushort objectIndex)
    {
        ...
        {
            StarfallStudio.Log.Verbose($"GetSpawnFlagsByIndex {objectIndex} {spawnFlags}");
            return spawnFlags;
        }
