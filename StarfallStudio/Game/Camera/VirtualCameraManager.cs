...
public class VirtualCameraManager : IDisposable
{
            ...
            if(virtualCamera is null)
            {
                switch(cameraType)
                {
                    case CameraType.Free:
                        ...
                        // preserve the rotation from a StarfallStudioCamera if switching to a free cam
                    ...
                    case CameraType.Cutscene:
                        ...
                        break;
                    default:
                        StarfallStudio.Log.Error($"Unknown camera type: {cameraType}");
                        break;
                }
            }
            else
    ...
    public (bool, int) CloneCamera(int cameraID)
    {
        ...
        if(_entityManager.TryGetEntity("cameras", out var ent))
        {
            if(cameraID == 0)
            {
                StarfallStudio.Log.Verbose("Cloning default camera.");

                var defaultCam = GetDefaultCamera();
                if(defaultCam == null)
                {
                    StarfallStudio.Log.Error("Default camera not found.");
                    return (false, -1);
                }
            ...
            {
                StarfallStudio.Log.Verbose($"Cloning camera {cameraID}");

                if(!_createdCameras.TryGetValue(cameraID, out CameraEntity? oldCamEnt))
                {
                    StarfallStudio.Log.Error($"Camera with ID {cameraID} not found");
                    return (false, -1);
                }
            ...
            {
                StarfallStudio.Log.Error("No camera found to clone");
                return (false, -1);
            }
        ...
        StarfallStudio.Log.Error("No camera container found");
        return (false, -1);
    }
    ...
    public bool DestroyCamera(int cameraID)
    {
        ...
        StarfallStudio.Log.Verbose("Destroying StarfallStudio camera " + cameraID);
    ...
    public unsafe void Update(MouseFrame* mouseFrame)
    {
        ...
        if(InputManagerService.ActionKeysPressed(InputAction.FreeCamera_Left))
            if(CurrentCamera.IsFreeCamera)
                if(ConfigurationService.Instance.Configuration.InputManager.FlipKeyBindsPastNinety && (CurrentCamera.PivotRotation < StarfallStudioUtilities.DegreesToRadians(-90) || CurrentCamera.PivotRotation > StarfallStudioUtilities.DegreesToRadians(90)))
                    leftRight += 1;
                else
                    leftRight -= 1;
            else
