...
using StarfallStudioRenderCamera = FFXIVClientStructs.FFXIV.Client.Graphics.Render.Camera;
using StarfallStudioSceneCamera = FFXIVClientStructs.FFXIV.Client.Graphics.Scene.Camera;
...
public unsafe class CameraService : IDisposable
{
    ...
    private delegate nint CameraCollisionDelegate(StarfallStudioCamera* a1, Vector3* a2, Vector3* a3, float a4, nint a5, float a6);
    private readonly Hook<CameraCollisionDelegate> _cameraCollisionHook = null!;

    private delegate nint CameraUpdateDelegate(StarfallStudioCamera* camera);
    private readonly Hook<CameraUpdateDelegate> _cameraUpdateHook = null!;

    private delegate nint CameraSceneUpdate(StarfallStudioSceneCamera* gsc);
    ...
    {
        return (StarfallStudioCamera*)CameraManager.Instance()->GetActiveCamera();
    }
