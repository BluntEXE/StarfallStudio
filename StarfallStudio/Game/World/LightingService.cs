...
//
// StarfallStudio's lights would not be possible without the help of the following projects:
...
namespace StarfallStudio.Game.World;
...
public unsafe class LightingService : IDisposable
{
    ...
    public unsafe bool ToggleLightDetour(EventGPoseControllerEX* state, uint index)
    {
        ...
        // It has issues like when using the Gpose Load/Save light preset it will desync from StarfallStudio
    ...
    public unsafe void SpawnLight(LightType lightType)
    {
        _framework.RunOnFrameworkThread(() =>
        {
            ...
            {
                StarfallStudio.Log.Debug($"GameLight addres: {gameLight.Address}");
            }
    ...
    public unsafe GameLight* SpawnGameLight(LightType lightType)
    {
        ...
        if(_virtualCameraManager.CurrentCamera is not null)
        {
            ...
            {
                light->Transform.Position = _virtualCameraManager.CurrentCamera.StarfallStudioCamera->Position;
                light->Transform.Rotation = _virtualCameraManager.CurrentCamera.StarfallStudioCamera->CalculateDirectionAsQuaternion();
            }
        }
    ...
    public void Clone(IGameLight sourceLight)
    {
        ...
        {
            StarfallStudio.Log.Error("Cannot clone an invalid or null light.");
            return;
        }
        ...
        _framework.RunOnFrameworkThread(() =>
        {
            ...
            foreach(var gameLight in _spawnedLights.AsEnumerable())
            {
                StarfallStudio.Log.Debug($"Cloned GameLight address: {gameLight.Address}");
            }
        });
    }
    ...
    public LightData? SaveLight(IGameLight light)
    {
        try
        {
            ...
            {
                StarfallStudio.Log.Warning("Cannot save an invalid or null light.");
                return null;
            }
        ...
        {
            StarfallStudio.Log.Error("Failed to save light.", ex);
        }
    ...
    public void LoadLight(LightData lightData, IGameLight igameLight, Vector3? centralPosition = null)
    {
        try
        {
            ...
            {
                StarfallStudio.Log.Warning("No light data to load.");
                return;
            }
            ...
            StarfallStudio.Log.Info($"Light loaded from {igameLight.Index}");
        }
        catch(Exception ex)
        {
            StarfallStudio.Log.Error("Failed to load light.", ex);
        }
    }
