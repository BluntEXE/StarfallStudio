...
public class ActorAppearanceCapability : ActorCharacterCapability
{
    ...
    public async Task<StarfallStudioApiResult> LoadMCDF(string path)
    {
        try
        {
            if(_mCDFService.IsApplyingMCDF)
            {
                StarfallStudio.NotifyError("Another MCDF is loading, Please wait for it to finish.");
                return StarfallStudioApiResult.IsApplyingMCDF;
            }
        ...
        {
            StarfallStudio.Log.Warning(ex, "Exception while Loading MCDF");
            StarfallStudio.NotifyError("MCDF Loading failed! Try again!");
        ...
        if(HasMCDF)
            return StarfallStudioApiResult.Success;

        return StarfallStudioApiResult.UnknownError;
    }

    public async Task<StarfallStudioApiResult> SaveMcdf(string path, string dis)
    {
        try
        {
        ...
        {
            StarfallStudio.Log.Warning(ex, "Exception while Loading MCDF");
            StarfallStudio.NotifyError("MCDF Export failed! Try again!");
    ...
    {
        StarfallStudio.Log.Debug($"Setting appearance for gameobject {GameObject.ObjectIndex}...");
        ...
        StarfallStudio.Log.Debug($"Appearance set for gameobject {GameObject.ObjectIndex}.");
    }
    ...
    public unsafe void ExportAppearance(string file)
    {
        ...
        StarfallStudioHuman.ShaderParams* shaders = Character.GetShaderParams();
