...
public class ActionTimelineCapability : ActorCharacterCapability
{
    ...
    {
        StarfallStudio.Log.Verbose($"StopSpeedAndResetTimeline {postStopAction is not null} {resetSpeedAfterAction}");

        var oldSpeed = SpeedMultiplier;

        SetOverallSpeedOverride(0);

        StarfallStudio.Log.Verbose($"SetOverallSpeedOverride {oldSpeed} {SpeedMultiplier}");
